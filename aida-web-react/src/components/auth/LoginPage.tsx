...
export function LoginPage() {
  const { login, mode, beginOidcLogin } = useAuth();
  ...
  async function onOidcSignIn() {
    ...
    try {
      ...
      setError('Failed to start OIDC login');
      setSubmitting(false);
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-surface px-4">
      <div className="w-full max-w-md rounded-xl border border-edge bg-surface-alt p-6 shadow-lg">
        ...
        <p className="mt-1 text-sm text-content-secondary">
          Auth mode: <span className="font-mono">{mode}</span>
        </p>

        {mode === 'oidc' ? (
          <div className="mt-5 space-y-4">
            {error && <p className="text-sm text-red-500">{error}</p>}
            <Button type="button" className="w-full" disabled={submitting} onClick={onOidcSignIn}>
