...
/// current process. See the module-level "Escape hatch" note. trace:STORY-684
...
/// editing those is legitimate advisor work. trace:STORY-684
...
/// and does NOT trip the gate. Case-insensitive on the extension. trace:STORY-684
    ...
    I: IntoIterator<Item = &'a str>,
{
    ...
    if auto_complete || solo_active || override_set {
        return None;
    }
...
/// unit-testable and the decision logic stays terse. trace:STORY-684
...
/// TTL). Thin wrapper over the presence module. trace:STORY-684
...
/// a guardrail, not a security wall). trace:STORY-684
...
mod tests {
    ...
    fn refuse(role: &str, paths: &[&str]) -> Option<String> {
        refusal(role, paths.iter().copied(), false, false, false)
    }
    ...
    fn auto_complete_lets_advisor_commit_code() {
        ...
        assert!(
            refusal("advisor", ["src/main.rs"].into_iter(), true, false, false).is_none(),
    ...
    fn solo_mode_lets_advisor_commit_code() {
        assert!(
            refusal("advisor", ["src/main.rs"].into_iter(), false, true, false).is_none(),
    ...
    fn explicit_override_lets_advisor_commit_code() {
        assert!(
            refusal("advisor", ["src/main.rs"].into_iter(), false, false, true).is_none(),
