// `aida defer` / `aida undefer` command cluster, lifted verbatim out of
// `main.rs` (SPIKE-78 pure-movement extraction). The deferred-trigger list
// renderer (`print_deferred_triggers`) stays in `main.rs` because the `aida
// list` views also consume it; this module reaches the shared helpers
// (`not_found`, `record_role_activity`) via `crate::`.
...
use crate::{not_found, record_role_activity};
