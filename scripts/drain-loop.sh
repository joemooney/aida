...
log "start (chunk=$CHUNK max-failures=$MAXF idle=${IDLE}s mode=$NOHUMAN)"
...
while true; do
  ...
  #    to NeedsAttention, advance the head) instead of halting. `|| true` so a
  #    non-zero exit never kills the loop.
  log "draining next${CHUNK}"
  aida queue work "next${CHUNK}" --auto-complete $NOHUMAN --max-failures "$MAXF" || true
