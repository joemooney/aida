...
## Session 62 — Autonomy keystone validated + a newcomer-adoption docs push (2026-06-09)

**The autonomy keystone shipped and was proven end-to-end.** Completed the burndown-machinery chain the operator chose to drive: **SPIKE-51** (empirical — headless `claude -p "/aida-burndown …"` resolves the disable-model-invocation skill AND its Agent-tool worktree fan-out executes under `-p`; **Strategy B confirmed**, no Rust reimplementation — verified on CLI 2.1.169, overruling a docs-based guide-agent that was wrong on all three points; memory `reference_headless_claude_p_skills_and_fanout`), **STORY-546** (#716 — the queued gate: `burndown plan` ready set now requires queue membership = advisor sign-off, + `--candidates` curation view), **STORY-545** (#718 — `aida burndown run`: the kick-off-and-walk-away headless drain over the blessed set, bypass-permissions default + `--concurrency`). Earlier in the day **STORY-547** (#715 — `aida burndown explain` + `aida why <ID>`, the derived "why is each open spec still open" view) answered the operator's vehicle question (derive-don't-store; reconciled with the broader STORY-548 rather than rejecting it).

**First real `aida burndown run` validated the keystone.** Drove **TASK-720** (positioning doc) end-to-end unsupervised: worktree-isolated implementer → PR #719 → CI green → squash-merged → auto-bumped Completed; punt-and-continue correctly parked the 3 gated specs. Inspected every claim independently. The drain's output (`vs-agent-teams.md`) is **excellent** — anti-overclaim, incentive-anchored, house-voice — strong evidence the keystone produces quality, not slop. One papercut filed: **TASK-721** (merged implementer worktrees aren't auto-pruned).

**Newcomer-adoption + Lane-D competitive response (operator's "make AIDA fantastic for newcomers, run autonomously, don't prompt me").** Mined the competitive research and ran with the safe doc-shaped moves. **#720** (merged): `positioning/when-not-to-use-aida.md` (honest scope, 6 cases), `positioning/composition.md` (use-AIDA-with-X recipes), `vs-aider.md` + `vs-continue.md` (editor neighbors, grounded in the verified 2026-05-26 scan), the `2026-06-09-positioning-recommendation-index-to-lifecycle.md` (Lane-D Kill-shot-3 response — **a recommendation, not a change**; filed as **CR-1-116**, operator picks A/B/C/keep), and a presentation-deck freshness pass (v0.11.0→v0.12.0 + headline numbers). **#721**: README "How AIDA compares" re-curated to lead with the two *nearest competitors* (Spec Kit, Kiro) + Agent Teams + the decision-aids (they were absent — backwards), and a new `docs/README.md` journey-ordered index (21 docs had none). Tracked as TASK-722 (Completed) under STORY-107.

**Judgment notes.** Held the tagline/VIS-1 change as operator-decision (CR-1-116) rather than unilaterally rewriting the brand. Deferred `vs-cursor.md` (no verified Cursor scan — refused to author from stale knowledge). **Skipped** a "Day 1" doc + `aida-tutor` as slop (first-project.md + /aida-onboard + aida-demo.sh already cover hands-on onboarding). Kept hands off orchestrator/authority/lifecycle code (the keystone runner itself shipped keyboard-supervised, not via unsupervised drain). Worked in an isolated worktree to avoid colliding with the live drain. Stopped at the genuine value frontier instead of manufacturing low-value positioning docs.

**For the operator on waking — highest leverage:** (1) **Decide CR-1-116** (the "missing index" headline — A: qualify "…of intent, not just code"; B: "specs and code stay permanently linked"; C: lifecycle-truth; recommend A/B) — touches README + VIS-1. (2) **Review the two held draft PRs** #709 (TASK-715 schema-over-MCP, needs schema.rs rebase) + #714 (STORY-543 lock-tolerant cache, `aida list --json` shape change). (3) **TASK-721** worktree-prune papercut (quick). (4) The highest-leverage *code* adoption move from the scan is **TASK-0416** (Spec Kit → AIDA import bridge) — supervised, it's the warm on-ramp from the #1 competitor; TASK-0417 (EARS lint) / TASK-0418 (context-grounding scan) are the other Lane-A adopt-adapt candidates.

## Session 63 — Backlog reset + the novice-first friction loop (2026-06-10, `/loop` self-paced)

**The pivot.** The operator hit a wall — *"running on fumes… each time I look at the backlog and cannot articulate a way forward I feel trapped."* They asked to clear the backlog and refocus on a simple, masterable product for a true novice (someone who's never heard of Spec Kit). **Cleared the board:** rejected the seed junk (TASK-13), **archived ~140 open specs** (all the accreted competitive/strategic/multi-agent/infra ambition — recoverable, code stays on main), closed the two held draft PRs (#709/#714, recoverable). Left **one** item (VIS-1). Applied **tagline A** (CR-1-116, operator-chosen): "missing index — *of intent, not just code*" across README/OVERVIEW/VIS-1 (#723). Recorded the P3 inter-agent-comms decision (STORY-550/SPIKE-52: ride Agent Teams within-Claude-session, OWN the cross-vendor mailbox — Agent Teams is Claude-only; memory `feedback_ride_native_within_vendor_own_cross_vendor`) and filed SPIKE-54 (ride Anthropic's just-shipped **advisor tool** for the implementer→advisor escalation hop — a term-collision with AIDA's advisor *role*, but complementary + validating, not a threat).

**The method (operator's idea, reframed): be the beginner, on a loop.** Fresh repo → use AIDA as a brand-new user → write down every stumble → fix the worst → throw it away → start fresh → repeat. The friction *is* the backlog, and it empties itself. Nine rounds, each surfacing a wall I'd never have caught reading code:

- **R1–2** (TASK-725, #728): `aida init` greeting was a jargon firehose (distributed mode / orphan branch / node id / forge / 21 discipline guides / teammate-onboarding). Rewrote to a warm minimal greeting (the 3-step loop + the default role); gated all the setup-detail behind `--verbose`.
- **R3** (TASK-727, #731): **showstopper** — the greeting's *own first suggestion* `aida add "Add a task from the CLI"` ERRORED (no positional title). Added an optional positional `[TITLE]` and defaulted a no-type add to **task** (so `aida add "do X"` → `TASK-1`, not `FR-2 Functional Requirement`).
- **R4** (TASK-726, #729): the code↔spec link is the whole magic but was undiscoverable — `aida show` reported "no linkage" and dead-ended. Now it says *how* (`// trace:<ID>` / `(ID)` in a commit), using the real id.
- **R5** (BUG-487, #730): the commit hook nagged a *hand-coding* novice to add `[AI:tool]` for a plain `trace:` (no `ai:` marker). Gated the nag to AI-authored traces.
- **R6** (TASK-727→`aida done`, #731): a solo novice **could not mark a task done** (`aida done` didn't exist; `edit --status completed` is jargon + TTY-gated). Added `aida done <SPEC>` — completes the capture→build→link→**done** loop.
- **R7** (TASK-728, #732): the bare-`aida add` type-picker led with `functional/principle` jargon (`task` buried at #11) → reordered; the init greeting now shows the full loop incl. `aida done`.
- **R8** (BUG-488, #733): `aida search` leaked the seeded META AI-prompt specs that `aida list` hides → consistent now (`--include-meta`).
- **R9** (TASK-729, #735): aligned `getting-started.md` to the new simple flow (`aida add "X"` / `aida done` / TASK ids); left `first-project.md` (deliberate full-graph walkthrough).

**The self-corrected mistake.** R8 (#733) merged with a **red Build** — I ran `gh pr merge` in the same breath as reading `FAILURE`, and the redness was a `trace:` marker inside a `///` clap doc (the both-at-once trap I'd already fixed once). Caught it the next iteration, fixed-forward (#734), main green within minutes. Two memories: `feedback_verify_ci_green_before_merge` (gh pr merge does NOT gate on CI; read the conclusion in a *separate* step before merging) and `feedback_file_spec_before_tracing` (file the spec first, trace with the *returned* id — guessing was off-by-one twice).

**Convergence.** A fresh end-to-end run is now smooth at every step: welcoming `init` → `aida add "X"` → clean `list` → build+link (no false nag) → `aida done` → recall shows the git linkage. The first-session mission is **done**; the only residual is cosmetic (the "N requirements" footer). The next frontier, when wanted, is the *growing-project* experience (a novice's 2nd session, organizing/picking-up across many tasks) — not yet explored.

**R10–14 — the AI-onboarding surface.** The loop's last arc moved from the *human* novice to the *agent* a novice hands the project to. Found and fixed the skill-template drift that fed agents commands the CLI rejects: `aida list --format summary` in `/aida-onboard` falsely reported *"No AIDA database found"* (BUG-489/490), and a systematic sweep (TASK-731, **#739/#740**) caught `aida list --format X` / `aida feature list` / `aida export --format markdown` / `requirements.yaml` / `aida-desktop` references across **10** skill + command templates. The `/aida-req` + `/aida-onboard` context blocks now surface `aida list open` (what's-left) so an agent leads with the right frame.

**R15 — the honest stop.** Full-binary smoke test (freshly-rebuilt release) confirms the whole cumulative chain holds: `init` → `add "X"` (→TASK-1) → `add "X" --type bug` (→BUG-2) → `list open` filters → `done TASK-1` (`✓ TASK-1 — done`) → `list` shows Completed + the *what's-left* tip → `show` prints the substituted `// trace:BUG-2` how-to hint. **Open backlog scanned in full (not a filtered view, per the standing discipline): only VIS-1 remains — no open *testable* novice work left.** The two genuinely-unexplored frontiers — the AI-assisted `/aida-implement` flow and the **TUI** (AIDA's public face) — both need *interactive* sessions that a non-interactive bash loop can't faithfully simulate. Rather than manufacture trivial fixes past the value frontier, **the loop stops here.** The testable first-session + AI-onboarding surfaces are comprehensively addressed; the interactive frontiers are the operator's call for a hands-on session.

## Session 64 — Surface unread mailbox into the agent's context: the read/notice half (2026-06-13, `/aida-pickup STORY-585`)

**The gap.** The mailbox WRITE side works (`send_message`), but an interactive agent never *notices* unread mail unless it manually runs `aida mailbox inbox` — which it never does unprompted. Messages (incl. STORY-569 `--zen` handoffs) sit unread while the operator watches the statusline `mail:1` nag wondering why the agent hasn't acted. STORY-585 is the **read/notice half** of that loop.

**Scope call (operator-confirmed).** The story splits cleanly into a *notice* half and an *interpret* half. Shipped the notice half; filed the interpret half (message intent markers `fyi|request|handoff` + an act-vs-prompt `[mailbox]` policy — a `Message` schema change + autonomy-mode wiring) as child **TASK-782** with the operator's folded-in 2026-06-12 refinements as its acceptance. Re-scoped STORY-585's acceptance to #1–5 (the notice half) via a recorded comment.

**What shipped (the substrate-as-bouncer fix).**
- **Pure core** (`aida_core::mailbox`): `unread_inbox()` + `build_notice()` (identity-union dedup, max-watermark unread semantics so reading as *any* identity clears it, capped with overflow). Six new unit tests incl. the role-addressed-handoff case invisible to the shell user.
- **`aida mailbox inbox --peek` (`--no-mark`) + `--unread`** (acceptance #1/#4): peek surfaces *without* advancing the watermark; plain `inbox` stays the explicit read+ack.
- **`aida mailbox notice`** — capped, plain, role+user-scoped unread summary, or nothing when caught up. The "slice verb" the hook *calls* (TASK-736 skill↔CLI symmetry).
- **`aida-mail-notice.sh` hook** wired on **SessionStart + UserPromptSubmit** (acceptance #2) — a 3-line dash-safe relay around the verb; plain stdout → `additionalContext` every turn, self-clearing on ack.
- **`/aida-read-mail` skill** (acceptance #3) — peek → interpret intent → read/ack → act, framed: *mail is interpreted input, not a command; reading is not obeying.*
- **Identity union {shell user, `AIDA_SESSION_ROLE`}** (acceptance #5) via a shared `inbox_identities()` helper, routed through *both* the notice and the statusline urgent counter — fixing the latent gap where a `--to advisor` handoff was invisible to the statusline's `current_user_id`-only resolution.
- **MCP `read_inbox` parity** (STORY-82): `unread` filter + `mark_seen` (default false = non-marking peek, matching the CLI split).
- Refreshed `docs/mailbox.md` (new read/notice section; stale "limitations" corrected — overview/urgency/retract all shipped).

**Verification.** 17 core mailbox tests + CLI `render_mailbox_notice` + MCP `read_inbox` peek/unread/mark_seen + the full MCP stdio suite all green; `cargo fmt --check` clean; clippy correctness clean. E2E in a throwaway agent identity: notice → non-marking peek → *still* shows → plain `inbox` acks → notice silent. Hook verified under `/bin/sh` (dash). **Gotcha hit + worked around:** the shared `CARGO_TARGET_DIR` (all worktrees → `aida/target`) was poisoned by a sibling's in-progress `skill-cli-symmetry.md` (TASK-736) embed; isolated my builds to a worktree-local `./target` (TASK-0396 family). Left the sibling's `aida-integrate` symlinks out of this PR (not mine).

## Session 65 — Marathon advisor drive: keystone reliability batch + solo-mode foundations (2026-06-13/14, `/loop` standing mandate)

A long advisor-seat marathon: review/merge the fleet's PRs, run the burn-down, answer design forks, and — under a standing *"implement persistently, never idle, decide on my behalf"* mandate — drive the open backlog down. **~25 PRs shipped; `main` verified green (2476 tests, 0 failed) at the end.**

**The supervised marker (STORY-610) + keystone-not-drain discipline.** The session's spine: keystone/reliability work (the drain machinery itself) ships at the keyboard, *never* in an unsupervised drain. Slice 1a excluded `supervised`-tagged specs from the drain ready set; slice 1b surfaced them in their own `burndown plan` "work these at the keyboard" section; BUG-551 fixed it to only list *actionable* (matching-status) ones, and moved the supervised collection after the status filter. This ended the recurring "how do I make progress on the awaiting-sign-off lump?" confusion by showing the route per spec.

**Keystone reliability batch (merged at the keyboard, operator-reviewed).** BUG-548/552 (force-push-to-protected-branch guardrail in `aida-git-guardrails.sh` — blocks `--force`/`-f`/`--force-with-lease` to `main|master|develop|aida-store`, matching the branch as a distinct arg and short `-f` anywhere); BUG-549 (drain subagents run `aida queue done` so the auto-bump fires); BUG-538 (drain concurrency guard — one drain per repo via `.aida/drain.lock` with pid-liveness + stale-reclaim + `AIDA_DRAIN_FORCE`, surfaced by `aida burndown status`); BUG-541 (integrator waits for each PR's CI to terminal; lists stragglers); BUG-536 (squash-body trailers complete umbrella children); BUG-543 (rollup-complete epics surface as ready-to-close); BUG-550; TASK-803 (extracted the pure `burndown::classify_spec` helper); TASK-804/806 (`burndown run --verbose`, in-flight marking, `aida burndown status` discoverability).

**Doc-intent arc (EPIC-40/STORY-603, ADR-5 ratified).** Docs now *derive from spec intent* — TASK-794 (the documenter consumes the spec graph: `--help` + interface_changes + description), TASK-795 (machine-readable `doc-intent`/`trace` backlink markers that don't leak SPEC-IDs into user-facing text), TASK-796 (drift-guard asserts interface_changes are reflected), TASK-802 (`config show` surfaces `[contained]`). ADR-5 (docs-generated-from-intent) ratified → Completed once the arc implemented it.

**`aida human` made readable (the operator's primary surface).** The operator's blunt feedback — *"I honestly cannot read it and understand what to do"* — drove TASK-807/808: relabelled the `ungroomed → to-groom` bucket with a second-person description (*"a draft awaiting YOUR review — you're the advisor"*), and de-noised the umbrella bucket (in-flight epics are self-driving → hidden as `InProgress`; only childless epics → `decompose`). BUG-553: the reviews bucket no longer mis-attributes a spec to a stacked sibling's branch (`git linkage` now prefers the spec-named branch when a commit is on multiple branches). The surface went from unreadable to a clean, actionable, per-bucket route.

**The mailbox nag, fixed (BUG-555).** The "N unread" notice fired every turn and never cleared — `aida mailbox inbox` with no `--agent` marked seen for the shell user only, while the notice counted across {shell user, role}; role-addressed handoffs were invisible to the read. Fix: the no-`--agent` inbox spans the full `inbox_identities()` set, unions each identity's inbox, and advances each watermark. **Live-verified gone.** This is also the advisor↔integrator escalation channel solo mode depends on.

**Sandbox confine-model (STORY-612/615).** Operator-confirmed forks: bwrap-only PR scope; write-confinement (`--ro-bind / /` + rw worktree/caches); shared net with `[contained] allowed_hosts` egress + fail-closed preflight. STORY-615 added `[contained] managed_domains_only` → `sandbox.network.allowManagedDomainsOnly` in the generated settings.

**Solo-mode foundations (EPIC-43).** The floor for an unattended `aida queue integrate --watch`: TASK-812 (the integrator takes the drain lock — no double-drive), TASK-813 (the integrator *parks* `supervised`/keystone candidates for the human via a new `SkipHeldForHuman` verdict instead of auto-merging), TASK-814 (serialize-group MVP — the drain fans ≤1 per `serialize:<group>` per wave, the substrate fix for the file-collision class). Capped by TASK-815: `docs/solo-mode.md`, the runnable groom→implement→integrate loop, honestly framed (it composes verified pieces; run supervised first).

**Incidents + lessons (→ memories).** A force-push-to-`main` incident (a parallel session's `--force-with-lease` off a failed `cd`) clobbered a merged PR — recovered via `reset --hard` + re-push; it drove BUG-548 and the *never-force-push-main* memory. The stacked-branch class (a sequential agent in one worktree branching off the prior unmerged commit → spec↔branch mis-attribution + semantic rebase conflicts) drove BUG-553/554 + the serialize-group fix. New memories: use `aida pull` (not raw `git pull`) for the Done→Completed auto-bump; `env -u AIDA_SESSION_ROLE cargo test -p aida-cli` (else role-gated queue/mcp tests false-fail — nearly mis-reported as a combined-main break); no backticks in `--description` args (shell command-substitution silently corrupts the stored spec text); `git reset --hard origin/main` between sequential specs in one worktree.

**The honest stop.** After the keystone batch + doc-intent arc + aida-human + mailbox + solo-mode foundations, the safe-autonomous backlog was genuinely drained. The residue (typed `ConflictsWith` v2 over the `serialize:` tag MVP, the fragile strict read-confinement, the CLI-manual chapter review) needs operator design input or is keystone-supervised. Per the mandate, I surfaced the decisions and idled rather than manufacture marginal churn — and wrote this log as the documentation fallback.

## Session 66 — Probe exhaustion: the gate-vs-rule program + research QA + product polish (2026-06-18/19, `/loop` standing mandate)

**The spine: a complete, honest experimental program that overturned the paper's sharpest claim.** The session ran the gate-vs-rule ablation (P1, substrate-as-bouncer) to its terminus — five controlled cells, each pre-registering and falsifying one single-variable conjecture, all built as real headless `claude -p`/`codex exec` runs with deterministic grading: I1 (commit-format, trivial, Claude — 100%, falsified "rules just fail"); I2 (trace-coverage, *buried*, Claude — 100%, falsified **attention-distance**); cross-vendor I2 (Codex — 100%, falsified **vendor**); I3 (procedural record-step, trivial — 100%, falsified **invariant-type**); I4 (procedural, *complex multi-file*, rule buried among ~8 — 100%, falsified **task-complexity**). The disciplined terminus is methodological, *not* a sixth theory: **a clean ablation cannot reproduce rule-dropping at all** — the lone drop (the competitive bake-off, n=1) lives in a large-real-codebase/long-autonomy regime controlled designs structurally can't reach. The only remaining evidence path is **field telemetry** (filed as `SPIKE-67`), not another ablation. Twice I caught myself drawing a single-variable theory through the one bake-off point and falsified it; the third time I stopped — that restraint is the result. The whole arc is indexed at `docs/research/ablations/README.md` and woven through the theory paper P1/§13/§15.

**Research QA: red-team of the load-bearing claims (TASK-846).** Adversarial passes on P2/P4/P5 scoped them honestly: **P2** split (stable-names-necessary holds; rich-graph-pays-for-itself open + operator-confounded); **P5** strong form ("60%-learner beats static-80%") reduced to conjecture (the *tune* leg of record→compare→tune is unbuilt — AIDA ships a calibration *dashboard*, not a self-correcting estimator). **P4** found a real, code-verified bug: the HLC the paper credited for deterministic merge is **dead code** (zero non-test call sites); the real scalar merge was wall-clock LWW with an "ours-wins" (last-puller-wins) tie-break = non-deterministic across clones. Filed + fixed as **BUG-578** — a deterministic content-hash tie-break with an order-independence test (fails against the old code, passes with the fix) + the dead HLC removed. A clean verify-lore-against-code catch.

**Product: the niche feature finished + telemetry-driven papercuts.** `aida compete` reached feature-complete (judge + ranking + honest-diff log-fix BUG-575 + gate-mirrors-CI BUG-576 — the dogfood found compete's build+test gate was narrower than CI). Telemetry-grounded fixes: BUG-577 (`rel list` rejected `--spec` → ~50% error rate from agents carrying the flag over; added `--spec`/`--id` aliases); TASK-856 (`aida add` did a full-store save + cache rebuild per add → routed through single-row write, 1.82s→1.02s median); SPIKE-66 (flagship TUI absent from the newcomer `getting-started` view despite "the TUI is the product" framing — now surfaced). Paper made publishable-grade with web-verified citations (TASK-847). The gate-audit applied the terminus to AIDA's own ~38 gates: the evidence *validates* the design (most gates are authorization/concurrency/integrity = out of scope, or run at the real-repo CI boundary = regime b); the one live calibration is STORY-499's `--block` flip, which should be data-driven off its report-only field record.

**The honest stop (again).** ~25 PRs merged, combined-main healthy, no findings awaiting triage. The probe is exhausted at every level — experiments run, terminus reached, load-bearing claims red-teamed, findings applied back to the product. The residue is all operator-gated: SPIKE-67 (field study green-light), STORY-621 (reliability-critical forge migration — keyboard, not unsupervised), the TASK-403/TASK-440 advisor escalations, TASK-857 (deferred), TASK-848 (deferred — snapshot was 1 day old). Per the mandate I shifted to a slow re-scan cadence rather than manufacture marginal churn, and wrote this log as the fallback.

---

## Session 2026-06-26 — Legibility audit + perf drive: "is it actually being worked?" → shipped surfaces, then sub-second status/writes

**The operator's recurring frustration became the session's spine: AIDA's surfaces did not reflect reality.** Across the day the operator caught me idling on an open list that would not shrink ("you refuse to move forward… no way to understand why"), asked why my "inbox:26" never cleared, why epics read In-Progress with no children, and why `aida status` took 26 seconds. Each turned out to be a real legibility gap, and each became landed code. The through-line: a *status flag is not liveness*, and a *count that will not clear* means diagnose its source, not repeat the action.

**Legibility surfaces shipped.** `aida status <spec>` (STORY-694) — per-spec liveness with ● live / ⚠ STALE / flag-only + session/pid/elapsed, plus `aida why` flagging a stalled in-flight lease instead of "being worked" (BUG-623). `aida ps` (STORY-696) — the global running-work table + orphaned-In-Progress flags. The statusline "inbox:N" mystery resolved: it is the *draft-inbox depth* (ungroomed drafts), not unread mail — I had read the mailbox repeatedly while the real ask was groom the drafts; the genuine mailbox-count divergence got fixed too (BUG-625). Epic status became a *read-only rollup of children* (BUG-626) — epics auto-move to In Progress when a child starts, manual epic-status edits are rejected — killing the "childless In-Progress epic" / "Draft epic with shipping children" confusion as a class. The cache learned to self-heal on schema drift (BUG-627) after the operator hit a hard "no column named blocked" error caused by my own mixed-version concurrent agents.

**The perf drive — the same root cause on reads and writes.** `aida status` was 26s because it `backend.load()`'d the full 2488-spec store and made ~16s of `gh` PR/CI network calls; STORY-707 made the default a sub-second cache snapshot (0.039s) and moved the heavy diagnostics into `aida doctor` (`--full`/`--ci` keep the rich view). BUG-634 was the write-side twin: single-spec writes took ~13-20s — and the deeper cause (found by profiling past the spec's hypothesis) was the STORY-48 lease-enforcement block full-scanning every write *whenever any lease was active* — so my own 7-agent fan-out was making every write slow; the targeted-I/O fix dropped `aida edit` to 0.75s even under concurrent agents. `aida usage --slowest` / `--events` (STORY-709) added the perf-debug lens the operator asked for — and it immediately, independently confirmed BUG-634 (edit 3-9s, add 31s in the raw stream).

**Identity, decomposition, and coordination.** Case-insensitive user matching (TASK-951 — `Joe`==`joe`, folded at comparison only, BUG-89-safe); `aida db block list` cleaned up (aligned columns, merged contiguous ranges, "full" not "exhausted", TASK-950); the cross-machine alias problem captured + un-deferred (TASK-845). The operator's "decompose all 10 epics" became mostly *clearing*: EPIC-29/25/7 rejected (already operator-rejected, just stale-Draft), focused draft children filed for EPIC-49/53/46/24, EPIC-47/30 confirmed shipped — plus the EPIC-53↔EPIC-54 cockpit seam doc so the two TUI epics do not build the same gestures twice. Coordination findings: BUG-637 (two agents implemented BUG-634 concurrently — no spec-scoped claim makes In-Progress not a lock), BUG-628 (rollup undercounts archived children), BUG-629 (over-broad `///` hook).

**The EPIC-54 incident — the legibility gap made destructive.** Folding the redesign epics, I rejected EPIC-54 without checking for active work — and *could not have caught it* even with `aida session leases`, because the operator's separate `claude --resume` agent held a generic harness-worktree lease, not a spec-scoped one. I restored EPIC-54 and, when the operator's agent later crashed, located its conversation by grepping `~/.claude/projects` (116 EPIC-54 mentions, last active 11:59) to hand back the exact `claude --resume <uuid>`. Concrete, twice-over proof of why `aida ps` + spec-linkable fan-outs matter. Also rescued TASK-894 from a 26-hour stuck lease (a hung `claude --resume`, killed + re-driven).

**Docs + discipline.** CLAUDE.md updated (daily-use status/ps/usage commands; storage cache-self-heal + targeted-writes; queue-identity case-insensitivity; epic-status rollup), the EPIC-53 seam doc (`docs/plans/2026-06-26-epic-53-cockpit-seam.md`), and this log. ~20+ PRs merged across the session, combined-main healthy throughout. New standing discipline: **flip a spec to In-Progress when fanning its agent** so `aida list --status in-progress` is an honest live view — the durable answer to "what are you working on right now?"

## Session 2026-09-06 — BUG-878 phase-2 push guard

Picked up `BUG-878` from the implementer queue. The bug was that phase 2 could proceed to CI/review/teardown while the implementer worktree held commits that had never been pushed, so CI and reviewer phases observed a stale PR head and `aida session end` could remove the only worktree carrying the new commit.
...
## Session 2026-09-07 — STORY-976 welcome-back status digest

Picked up `STORY-976` from the implementer queue. The story asked `aida status` to detect a long local absence and lead with a welcome-back digest instead of requiring the returning operator to know which history, lease, and awaiting commands to run.

Added absence detection to both bare and rich `aida status`, with threshold resolution from `AIDA_ABSENCE_DAYS`, then `.aida/config.toml [status] absence_days`, defaulting to 14 days. The report computes last activity from the newest store commit, project event stream row, or scoped usage row; when over threshold it prints last activity in local time, counts created/completed/rejected changes from the structured `aida history` decoder, lists up to five changed titles, and shows live vs stale lease counts before the normal Awaiting-you section. `aida status --json` now includes top-level `absence_days` and `last_activity_at` plus an `absence` detail object, and `aida awaiting --notice` adds the requested “back after N days, run aida status” line when over threshold. Added `// trace:STORY-976 | ai:codex` to the status implementation.

Verification: `cargo fmt --all -- --check`; `cargo test -p aida-cli-lib story_976_absence_tests -- --nocapture`; `cargo test -p aida-cli-lib story_465_awaiting_report_tests -- --nocapture`; `cargo check -p aida-cli-lib`; `AIDA_ABSENCE_DAYS=0 cargo run -q -p aida-cli -- status --json --no-ci | jq '{absence_days,last_activity_at,has_absence:(.absence != null)}'`. Existing unrelated Rust warnings remain in the test output.

## Session 2026-09-07 — STORY-975 drain transient self-retry

Picked up `STORY-975` from the implementer queue. The story required a bounded drain-level retry for transient typed shelve causes so a watchdog/no-verdict/no-PR/tool-exit/cache-lock blip can recover once without a manual relaunch, while non-transient reviewer/CI/environment/internal outcomes still park immediately.
...
## Session 2026-09-07 — BUG-895 phase-3 missing PR recovery

Picked up `BUG-895` from the implementer queue. The bug was a BUG-893 follow-up: the orchestrator-created PR recovery only ran while the implementer worktree still existed, but the observed failure happened later at phase 3, after phase 2 had already pushed the branch and torn down the worktree. The reviewer preflight saw no PR number, constructed a `NoPr` failure, retried once via STORY-975, and parked without ever trying to open the PR from the pushed branch.
...
## Session 2026-09-08 — BUG-899 worktree submodule initialization

Picked up `BUG-899` from the implementer queue. The bug was that AIDA-created worktrees did not initialize recursive git submodules, leaving vendored dependencies as empty gitlinks and causing builds to fail after pickup.

Added a shared `git_ops::init_submodules_or_warn` helper that detects `.gitmodules`, runs `git submodule update --init --recursive` by default, and prints the exact recovery command when `[worktree] init_submodules = false` opts out. Wired the helper through session-start worktree creation, manual `aida worktree add|enter`, warm-pool create and reuse, integrator checkout reuse/acquire, compete arms, and temporary PR rebase worktrees. Added the `[worktree] init_submodules = true` scaffold section to all init paths and `// trace:BUG-899 | ai:codex` comments on the implementation and tests.

Verification: `cargo fmt --all -- --check`; `cargo check -p aida-cli-lib`; `cargo test -p aida-core acquire_populates_submodules_by_default`; `cargo test -p aida-core acquire_opt_out_leaves_submodules_uninitialized`; `cargo test -p aida-core submodule_init_command_names_recursive_recovery_command`; `cargo test -p aida-cli-lib worktree_handler`. Existing unrelated Rust warnings remain in the test output.

## Session 2026-09-08 — BUG-900 queue routing identity fracture

Picked up `BUG-900` from the implementer queue. The bug was that advisor/coordinator queue writes using `--for implementer` could report success while landing under the coordinator's personal queue identity, making the routed work hard for implementers to discover across distinct AIDA user ids.
...
## Session 2026-09-09 — STORY-991 agent launcher resume and duplicate prompts

Picked up `STORY-991` from the implementer queue. The story asked `aida agent new` to consult the agent registry after vendor and role selection, warn before launching a same-vendor/same-role live duplicate, and offer recent ended sessions for that vendor and role.
...
## Session 2026-09-09 — STORY-993 PID/TTY + three-state liveness in `session conversations`

Picked up `STORY-993` via `/aida-pickup STORY-993` (fourth attempt; the first two implementer sessions exited without a commit and the salvaged partial on branch `story-993` was not building, the third rebuilt from `origin/main` and committed but was ended before its PR, so this session cherry-picked that commit onto `story-993-4`, finished verification, and fixed a `/dev/null` stdin rendering as TTY `null`).

`aida session conversations` (and its deprecated `list` alias) now resolves a real process per row instead of painting `●` for any transcript written in the last five minutes. New `aida-core::liveness::probe_live_agent_processes` is ONE memoized `/proc` walk that classifies claude **and** codex processes and captures start time + controlling tty; the existing claude-only probe is now a projection of it, so a CLI run that needs both views still scans once. New `aida-cli-lib/src/session_liveness.rs` is the pure resolver: (a) the covering lease's `active_pid` (joined lease ↔ transcript through the session manifest's `claude_session_id`; `creator_pid` accepted only when it IS a live agent process), else (b) a same-agent process whose cwd equals the transcript's and whose start time lies in the transcript's life — not before its first event (−120s slack) and not after its most recent `SessionStart:startup|resume` hook event (+60s slack; `compact` does not count; read lazily by a backward 1 MiB chunk scan, capped at 32 MiB, only for rows with a cwd-matched live process). Two survivors = ambiguous → PID `?`, both pids in JSON, never `●`. The table gains PID and TTY columns, the AGE cell is process elapsed for `●` rows, the legend explains `●` / `◐` / blank, `--format json` carries `pid`, `pids`, `tty`, `liveness`, `resolution`, `elapsed_secs`, `lease_role`, and a transcript with no hook role takes the lease role (TASK-152 parity with `aida ps`). Docs: `docs/cli/06-roles-sessions.md`.

Verification: `cargo fmt --all -- --check`; `env -u AIDA_SESSION_ROLE cargo test -p aida-cli-lib -- session_liveness session:: story_696` (137 passed); `cargo test -p aida-core --lib liveness` (39 passed); `cargo clippy` gate; `make build-fast` + a timed `aida session conversations` / `--format json` run against the live machine (see PR).

## Session 2026-09-10 — BUG-913 no-launch vendor resolution

Picked up `BUG-913` from the implementer queue. The bug was that non-launching `aida queue work` paths could still display disabled Claude defaults in codex-only projects: `--no-launch --dry-run` skipped the enabled-vendor resolver, and `--no-launch` completion instructions hardcoded `claude /aida-pickup`.
...
## Session 2026-09-09 — BUG-914 worktree-local AIDA runtime excludes

Picked up `BUG-914` from the implementer queue because `STORY-995` was blocked by `STORY-994`. The bug was that AIDA-created worktrees in projects without tracked AIDA scaffolding could show `.aida/` and `.aida-store` as untracked, making coordination artifacts easy to stage accidentally.
