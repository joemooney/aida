...
Picked up `STORY-993` via `/aida-pickup STORY-993` (third attempt; the two earlier implementer sessions exited without a commit, and the salvaged partial on branch `story-993` was not building, so this was rebuilt from `origin/main`).
