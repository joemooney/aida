...
**Type protocols.** AIDA stores concise work contracts for spikes, bugs, stories, tasks, decisions, and docs as editable META requirements. Interactive pickup injects the matching protocol before work begins, cites its META id, caps it at 40 body lines, and labels the precedence `type protocol < spec acceptance`; a leased session receives the compact reminder again in its per-turn notice. Inspect them with `aida protocol list` and `aida protocol show <type>`; editing the META body changes the next pickup without rebuilding AIDA.
