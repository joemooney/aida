...
**The concrete embodiment.** `aida why <file:line>` answers why a line exists from a trace comment or conventionally trailered commit. `aida graph <id> --impact` shows what a requirement touches. `aida compete` can run a spec through multiple vendors in isolated worktrees and let objective gates plus review pick the strongest result. `aida spec dryrun` and `aida spec interview` tighten specs before implementation. The wedge is practical and machine-checkable: stable IDs, typed relationships, lifecycle state, and inline `// trace:SPEC-ID` links that connect code back to intent.
