...
- **Read liveness from the lease, never a process name.** Use `aida ps`, `aida burndown status`, or the drain lock's recorded PID + process start time. Never gate a wave launch with `pgrep`: wrappers can match their own command text, full-path launches evade name patterns, and recycled PIDs are not the process that acquired the lease.
