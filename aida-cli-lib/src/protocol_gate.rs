...
fn diff_item(changed: &ChangedFiles, is_hit: fn(&str) -> bool, none_detail: &str) -> ItemState {
    match changed {
        ...
        ChangedFiles::Known(files) => match files.iter().find(|f| is_hit(f)) {
    ...
    let items = kinds
        ...
        .map(|&kind| {
            let state = match kind {
                ...
                ItemKind::BugTestChange => {
                    diff_item(changed, is_test_file, "no test file in the diff")
                }
                ItemKind::AdrAcceptedWithReferences => {
                    ...
                    match (accepted, referenced) {
                        (true, true) => ItemState::Met("accepted, references edge present".into()),
                        (false, true) => {
                            ItemState::Missing(format!("status is {}, not accepted", req.status))
                        }
                        (true, false) => ItemState::Missing("no references edge".into()),
                        (false, false) => ItemState::Missing(format!(
                            "status is {}, not accepted; no references edge",
                            req.status
                        )),
                    }
                }
...
pub(crate) fn changed_files_for_branch(repo: &Path) -> ChangedFiles {
    ...
    match crate::harvest::changed_files_base_to_head(repo, &base) {
        Ok(files) => ChangedFiles::Known(files),
        Err(e) => ChangedFiles::Unknown(format!("{e:#}")),
    }
}
...
mod tests {
    ...
    fn known(files: &[&str]) -> ChangedFiles {
        ChangedFiles::Known(files.iter().map(|s| s.to_string()).collect())
    }
