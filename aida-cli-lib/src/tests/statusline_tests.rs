...
fn render_session_env_file_quotes_path() {
    let body = render_session_env_file(std::path::Path::new("/tmp/aida/target"), None, None);
    ...
    let body = render_session_env_file(std::path::Path::new("/tmp/joe's repo/target"), None, None);
    assert!(body.contains("export CARGO_TARGET_DIR='/tmp/joe'\\''s repo/target'"));

    let body = render_session_env_file(
        std::path::Path::new("/tmp/aida/target"),
        Some("codex"),
        None,
    );
...
fn parse_session_env_roundtrips_with_render() {
    let body = render_session_env_file(std::path::Path::new("/tmp/parent/target"), None, None);
...
/// BUG-766: the shim pins the coordinating binary — an `AIDA_BIN` export
/// plus a PATH prepend of its directory — so a shell that sources it (or a
/// child session launched from it) resolves bare `aida` to the build that
/// created the worktree, never a stale installed binary from the raw PATH.
// trace:BUG-766 | ai:claude
#[test]
fn render_session_env_file_pins_coordinating_binary() {
    let body = render_session_env_file(
        std::path::Path::new("/tmp/parent/target"),
        None,
        Some(std::path::Path::new("/repo/target/debug/aida")),
    );
    assert!(body.contains("export AIDA_BIN='/repo/target/debug/aida'"));
    assert!(body.contains("PATH='/repo/target/debug':\"$PATH\"\n"));

    // The narrow in-process parser must apply AIDA_BIN but SKIP the PATH
    // line (a plain assignment, not `export VAR='value'`) — mangling the
    // `"$PATH"` reference into a literal would poison the launched env.
    let pairs = parse_session_env(&body);
    assert!(pairs
        .iter()
        .any(|(k, v)| k == "AIDA_BIN" && v == "/repo/target/debug/aida"));
    assert!(pairs.iter().all(|(k, _)| k != "PATH"));
}
