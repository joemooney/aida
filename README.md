...
Karpathy's *LLM Wiki* showed that a folder of markdown you can **ask** beats a heavyweight app. AIDA adds the one thing a notes-wiki can't: your intent is **wired to the code**, so you can also ask *"what breaks if I change this?"* — `aida graph impact <id>`.
...
Two statuses are terminal **off-ramps**, and they mean opposite things. **Rejected** = *declined* — we looked and said no. **Superseded** = *adopted, then replaced* — this spec governed, and a later one now does. Record the successor with `aida edit SPEC --status superseded --superseded-by NEW-SPEC`; naming the successor is enough (the flag implies the status), and the link is stored as a real typed relationship you can walk with `aida graph`, not a string tag. The archetype is an ADR that was accepted and later replaced: before Superseded existed, it had to masquerade as Rejected, so a decision followed for months looked exactly like one turned down. Superseded renders as a dimmed, checked-box `⊡ Superseded` — never the red `✗ Rejected` — and, being terminal, drops out of the default `aida list` view and the open tally.
