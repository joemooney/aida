...
//! `Joe.Mooney@work.example` on a work host, `joe.mooney@gmail.com` on a personal
...
mod tests {
    ...
    fn chained_links_collapse_to_one_person() {
        ...
        reg.link("joe.mooney", "joe.mooney@work.example");
        reg.link("joe.mooney@work.example", "joe.mooney@gmail.com");
        // All four resolve to the single canonical person.
        let canonical = reg.resolve("joe");
        assert_eq!(reg.resolve("joe.mooney"), canonical);
        assert_eq!(reg.resolve("joe.mooney@work.example"), canonical);
    ...
    fn case_fold_and_alias_resolve_compose() {
        ...
        reg.link("joe", "Joe.Mooney@work.example");
        assert_eq!(reg.resolve("JOE.MOONEY@work.example"), "joe");
        assert_eq!(reg.resolve("Joe"), "joe");
    }
