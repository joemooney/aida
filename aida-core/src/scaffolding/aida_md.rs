...
pub(super) const MEMORY_REFLEX_BLOCK_BEGIN: &str = "<!-- AIDA-MEMORY-REFLEX-BEGIN -->";
pub(super) const MEMORY_REFLEX_BLOCK_END: &str = "<!-- AIDA-MEMORY-REFLEX-END -->";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MemoryReflexBlockMerge {
    Added,
    Refreshed,
}
...
impl Scaffolder {
    /// Generate the slim memory-lane reflex block injected into root agent
    /// instruction files. Keep this wording free of execution machinery
    /// vocabulary: it is for ordinary chat sessions that should read and
    /// update project memory without learning AIDA's larger workflow surface.
    // trace:STORY-1094 | ai:codex
    pub(super) fn generate_memory_reflex_block(&self) -> String {
        format!(
            "{}\n{}\n{}\n",
            MEMORY_REFLEX_BLOCK_BEGIN,
            r#"## AIDA Memory Reflex
...
/// Pull the AIDA memory reflex block out of a root instruction file.
/// Markers are line-anchored so prose may safely mention the marker names.
// trace:STORY-1094 | ai:codex
pub fn extract_memory_reflex_block(content: &str) -> Option<&str> {
    let begin = find_marker_at_line_start(content, MEMORY_REFLEX_BLOCK_BEGIN)?;
    let after_begin = begin + MEMORY_REFLEX_BLOCK_BEGIN.len();
    let end_rel = find_marker_at_line_start(&content[after_begin..], MEMORY_REFLEX_BLOCK_END)?;
    Some(content[after_begin..after_begin + end_rel].trim_matches(['\n', '\r']))
}

/// Merge the generated memory reflex block into a user-owned root instruction
/// file while preserving all prose outside the markers.
// trace:STORY-1094 | ai:codex
pub fn merge_memory_reflex_block(
    existing: &str,
    generated: &str,
) -> (String, MemoryReflexBlockMerge) {
    let generated_block = memory_reflex_marker_region(generated).unwrap_or(generated);
    if let Some(begin) = find_marker_at_line_start(existing, MEMORY_REFLEX_BLOCK_BEGIN) {
        let after_begin = begin + MEMORY_REFLEX_BLOCK_BEGIN.len();
        if let Some(end_rel) =
            find_marker_at_line_start(&existing[after_begin..], MEMORY_REFLEX_BLOCK_END)
        {
            let end = after_begin + end_rel + MEMORY_REFLEX_BLOCK_END.len();
            ...
            return (merged, MemoryReflexBlockMerge::Refreshed);
        }
    }
    ...
    (merged, MemoryReflexBlockMerge::Added)
}
...
fn memory_reflex_marker_region(generated: &str) -> Option<&str> {
    let begin = find_marker_at_line_start(generated, MEMORY_REFLEX_BLOCK_BEGIN)?;
    let after_begin = begin + MEMORY_REFLEX_BLOCK_BEGIN.len();
    let end_rel = find_marker_at_line_start(&generated[after_begin..], MEMORY_REFLEX_BLOCK_END)?;
    let mut end = after_begin + end_rel + MEMORY_REFLEX_BLOCK_END.len();
    if generated[end..].starts_with('\n') {
        end += 1;
    }
    Some(&generated[begin..end])
...
mod tests {
    ...
    #[test]
    fn merge_memory_reflex_block_appends_and_refreshes() {
        let generated = "<!-- AIDA-MEMORY-REFLEX-BEGIN -->\nnew\n<!-- AIDA-MEMORY-REFLEX-END -->\n";
        let (merged, action) = merge_memory_reflex_block("# CLAUDE.md\n\nUser notes.\n", generated);
        assert_eq!(action, MemoryReflexBlockMerge::Added);
        assert!(merged.contains("User notes."));
        assert!(merged.contains("<!-- AIDA-MEMORY-REFLEX-BEGIN -->\nnew\n"));

        let existing = "# top\n<!-- AIDA-MEMORY-REFLEX-BEGIN -->\nold\n<!-- AIDA-MEMORY-REFLEX-END -->\n# bottom\n";
        let (refreshed, action) = merge_memory_reflex_block(existing, generated);
        assert_eq!(action, MemoryReflexBlockMerge::Refreshed);
        assert_eq!(
            refreshed,
            "# top\n<!-- AIDA-MEMORY-REFLEX-BEGIN -->\nnew\n<!-- AIDA-MEMORY-REFLEX-END -->\n# bottom\n"
        );
