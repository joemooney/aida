...
"#
    ...
    /// MCP server with Codex CLI when the user opts in with
    /// `aida init --with-mcp`.
    ///
    ...
    /// hand after that explicit opt-in. Trust is deliberately absent — see BUG-793; it opts the project into
    ...
    /// when MCP registration is enabled).
    ///
...
mod tests {
    ...
    fn generated_agents_md_has_codex_operating_sections() {
        ...
        assert!(md.contains("aida init --with-mcp"));
        assert!(md.contains("costs about 2x"));
