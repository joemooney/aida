...
## Waiting for mail — zero model tokens while idle

Run this skill on demand or after the substrate reports actionable work. The
only supported unattended pattern is a **shell-side/event-driven wait** that
keeps the model asleep while nothing is ready: use the harness `Monitor` over
`aida watch --emit-wakes`, a background shell wait around `aida awaiting
--notice`, or the event-driven `aida advisor watch` heartbeat from TASK-1245.
Wake the model only after that process emits an actionable notice.

**Never use model-side `CronCreate`, `/loop`, or `ScheduleWakeup` to poll a
mailbox. Never stack recurring wake mechanisms.** Even a quiet one-line reply
reloads the session context and therefore consumes tokens. If an explicit,
one-off check finds no unread mail and nothing awaiting, report `✉ no unread
mail` once and stop; do not schedule another model turn.
