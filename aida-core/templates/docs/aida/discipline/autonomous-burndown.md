...
**The fan-out burndown is Claude-harness-only.** The parallel wave is spawned with the Claude Code harness's native subagent Task tool (`Agent(subagent_type: …, isolation: "worktree")`) — no CLI verb drives it, only the harness can. Non-Claude vendors (Codex, Gemini, …) have no equivalent subagent fan-out, so `/aida-burndown` cannot run there. Their vendor-neutral path is the per-spec `aida queue work <id> --auto-complete` engine (the orchestrator drain) run **serially** over the ready set — one spec's full implement → CI → review → merge lifecycle at a time. It trades parallelism for a loop that runs anywhere the `aida` binary does: **fan-out burndown = Claude harness; serial `aida queue work --auto-complete` = every agent, including non-Claude.**
