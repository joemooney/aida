// Native entry point - for desktop builds only
...
mod native_main {
    ...
    pub fn run() -> Result<(), eframe::Error> {
        ...
        let title = if cli.server.is_some() {
            format!(
                "AIDA - Requirements Manager (Remote: {})",
...
// WASM stub - the actual WASM entry point is in lib.rs via wasm_bindgen(start)
#[cfg(target_arch = "wasm32")]
fn main() {
    // This binary target is not used for WASM builds.
    // The WASM entry point is the #[wasm_bindgen(start)] function in lib.rs.
    // This stub exists to satisfy cargo when compiling with --all-targets.
}
