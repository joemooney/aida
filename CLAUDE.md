...
### React Dashboard
The React dashboard is located at `aida-web-react/` and runs on port 5173 (dev) via Vite, connecting to the REST API on port 8080. Stack: React 19, Vite 8, Tailwind CSS 4, @tanstack/react-query. Shared TypeScript types are generated from Rust structs in `shared/types.ts`. Views: Dashboard (project-wide status cards + active sprint summary + queue widget with clickable navigation to filtered List View), Kanban Board (with tag filter dropdown and active filter chips), List View (with flat/tree toggle for parent/child hierarchy, inline tag pills), My Queue (personal focus inbox with drag-to-reorder via @dnd-kit/sortable), Sprint Planning (with drag-and-drop backlog/sprint assignment), Timeline (chronological event feed from history/comments/creation), Skills Browser (view/edit skills and commands), Chat (AI-powered Q&A for PMs/stakeholders with streaming responses, full requirements context, and auto-linked spec IDs via LinkedMarkdown — requires `ANTHROPIC_API_KEY` env var), Settings (store metadata, relationship/type/reaction definitions, ID config, prefix management via `/api/v2/settings/` endpoints, Admin tab with dev-mode server rebuild & restart via SSE). Features: structured search (`owner:joe`, `tag:frontend` syntax in search bar), tag-based filtering across views, markdown description rendering in detail panel, "Add to Queue" actions in detail header and list rows.
