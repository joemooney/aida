...
**Reach for it when** — a reviewer (or you) requested changes on a Done spec and it needs another implementer pass. Also for any "this shipped wrong, reopen it" recovery. It is also the one-keystroke **requeue** for a spec parked in Needs Attention (a punt, a drain shelve, or an advisor escalation): once you have triaged it, `aida rework <ID>` moves it to Approved, clears the parking markers the drain left (including the `needs-human` escalation tag), records on the spec why it came back, and puts it back on the queue, so the next drain picks it up. `aida findings list` and `aida awaiting` print this command next to each parked spec. Returning a parked spec is an advisor decision, so it needs the advisor role or an interactive terminal, the same as `aida edit <ID> --status approved`. A spec with an unanswered decision stays parked until that decision is answered (`aida questions`).
