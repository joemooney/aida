...
**Gotchas.** `rel remove` removes only edges of the `--type` you name. Removing `references` (or `related`) also removes an old custom `related` edge to the same target. To repair every old custom `related` edge at once, use `aida db migrate-related-edges` (Chapter 10). `rel list` with no args lists *every edge in the graph* — on a large project that's a firehose; pass a spec-id (or `--source`/`--target`) to scope it. The `--source` flag is just the explicit form of the positional ID, for scripts that want to be unambiguous.
