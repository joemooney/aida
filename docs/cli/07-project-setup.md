...
- `--since <ref-or-date>` — exempts specs completed before a cutoff (a git ref/tag, or a time bound such as `30d`, `2w`, an ISO date at local midnight, or RFC3339) from the completed-without-commit check, to quiet noise on legacy history predating trace conventions. The way you stop `doctor` nagging about pre-AIDA commits.
