...
Around that core sit the **launchers** and the **comms**. `aida agent new` and `aida session new`/`start --launch` spawn an actual Claude/Codex process with the right cwd, env, role, and a startup context snapshot — the supervised way to put an agent on a spec rather than copy-pasting a prompt. And because a fleet of agents needs to coordinate without sharing scrollback, two file-backed channels carry the traffic: **briefs** (`aida brief` — one-directional "here's your next pickup," operator → agent) and the **mailbox** (`aida mailbox` — peer↔peer messaging between agents). The `aida advisor` family is the seam where the *live* advisor session registers itself so the headless orchestrator can fork its full context instead of cold-booting a stranger. Keep the three substrates distinct as you read: **leases** coordinate *work*, **briefs** route *assignments*, the **mailbox** carries *conversation*.
...
**Mental model.** There is one *master* advisor per project — the persistent live Claude session wearing the advisor hat. `aida advisor` is the seam that lets the rest of the system *find* it. `aida advisor register` writes `~/.aida/advisor.toml` recording the current Claude session as the live advisor; the `--no-human=both` orchestrator reads that file to decide whether to **fork the live advisor** (inheriting its full in-flight context) or **cold-boot** a fresh headless advisor (same model, empty context). Registration is what turns "a stranger handles the punt" into "your advisor, with everything it knows, handles the punt." Beyond registration, the family carries the advisor's *operational* loops: a situational dashboard, a presence-gated watch loop, and recurring scheduled tasks.
...
- `watch --dry-run` / `--once` / `--triage-only` — `--dry-run` previews each tick's decision *and the fork cost* without forking (forks are not free — both runs fire); `--once` is the cron-friendly single tick; `--triage-only` is the conservative "surface, don't act" mode. `--fork-interval` / `--poll-interval` tune cost vs responsiveness.
...
**Gotchas.** The forked headless advisor (`watch`, and the `--no-human=both` escalation tier) is a *fork of your context*, but the cold-boot fallback is a stranger — registration is what makes the difference, and an unregistered advisor session silently gets the worse path. The watch loop's forks have real cost (both the cold-boot drive and the fork shadow can fire); `--dry-run` exists precisely so you can see that cost before committing.
