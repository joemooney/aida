...
**Mental model.** The advisor seat is the persistent live Claude session wearing the advisor hat; `aida advisor` is the seam that lets the rest of the system *find* it and run its maintenance loops. `aida advisor register` writes `~/.aida/advisor.toml` recording the current Claude session as the live advisor. That registration enables the **fork-from-live** path: `aida advisor watch` can copy-then-resume your session's JSONL so a headless pass boots with your *full in-flight context* instead of a stranger's. Note the deliberate split between the two headless-advisor paths — they are **not** the same mechanism:

- **fork-from-live** — used by `aida advisor watch` (the away loop). Requires registration; inherits your live context. ~$0.03 warm.
- **cold-boot** — used by the `--no-human=both` *in-drain escalation tier* (`/aida-advise`) when an implementer punts mid-drain. Same model, but loads only the *persistent* substrate (memory, CLAUDE.md, discipline, spec graph) — **none** of your in-flight conversational context.
