...
A **session** is a unit of work with a *lease* on a *scope*. When an implementer picks up `<spec-id>`, `aida session start` spins a sibling git worktree on a fresh branch and writes a **lease** — a small file that says "this session owns `<spec-id>`, on this branch, in this worktree, held by this owner." The lease is the collision-avoidance substrate: before two agents both grab the same spec, `aida session leases` answers "who holds what right now?" so the second one routes around it. v1 leases are *advisory* (not yet hard-enforced by `edit`/pickup), but in a multi-agent drain they are the coordination contract that keeps the fan-out from double-driving a spec. Note the deliberate split between `aida session leases` (active *scoped work* — the "what's running" view) and `aida session list` (historical *Claude Code conversations* — a different, .jsonl-backed concept); reach for `leases` when you mean "who's working."

Around that core sit the **launchers** and the **comms**. `aida agent new` and `aida session new`/`start --launch` spawn an actual Claude/Codex process with the right cwd, env, role, and a startup context snapshot — the supervised way to put an agent on a spec rather than copy-pasting a prompt. And because a fleet of agents needs to coordinate without sharing scrollback, two file-backed channels carry the traffic: **briefs** (`aida brief` — one-directional "here's your next pickup," operator → agent) and the **mailbox** (`aida mailbox` — peer↔peer messaging between agents). The `aida advisor` family is the seam where the *live* advisor session registers itself so the away **watch loop** can fork its full context (the in-drain escalation tier stays cold-boot — see `aida advisor` below). Keep the three substrates distinct as you read: **leases** coordinate *work*, **briefs** route *assignments*, the **mailbox** carries *conversation*.
...
- `aida session list` / `resume` — finding and re-entering a *past conversation* by its role+spec context instead of grepping Claude's auto-generated subjects.

**Don't reach for it when** — you want the "who's working" view but typed `list` (that's historical conversations — use `leases`). And don't `start` a scoped worktree for trivial in-place work; the worktree-per-scope model pays off for *parallel* or *long-running* work, not a two-line fix on your current branch.
...
**Mental model.** The advisor seat is the persistent live Claude session wearing the advisor hat; `aida advisor` is the seam that lets the rest of the system *find* it and run its maintenance loops. `aida advisor register` writes `~/.aida/advisor.toml` recording the current Claude session as the live advisor. That registration enables the **fork-from-live** path (SPIKE-11): `aida advisor watch` can copy-then-resume your session's JSONL so a headless pass boots with your *full in-flight context* instead of a stranger's. Note the deliberate split between the two headless-advisor paths — they are **not** the same mechanism:

- **fork-from-live** — used by `aida advisor watch` (the away loop). Requires registration; inherits your live context. ~$0.03 warm.
- **cold-boot** — used by the `--no-human=both` *in-drain escalation tier* (`/aida-advise`, STORY-306) when an implementer punts mid-drain. Same model, but loads only the *persistent* substrate (memory, CLAUDE.md, discipline, spec graph) — **none** of your in-flight conversational context.

Registration's payoff is concentrated in the watch/away loop; the in-drain escalation tier is cold-boot today regardless. Beyond registration, the family carries the advisor's *operational* loops: a situational dashboard, the watch loop, and recurring scheduled tasks.
...
**Gotchas.** Don't conflate the two headless-advisor paths: only `aida advisor watch` *forks your live context* (and only if you registered); the `--no-human=both` in-drain escalation tier is *cold-boot* (`/aida-advise`) — same model, only the persistent substrate, none of your conversation. Registration is what enables the fork path for `watch`; an unregistered session gets cold-boot. And the watch forks aren't free (~$0.03 warm each, over the away window) — `--dry-run` exists precisely so you can see the cost before committing.
