...
- `new claude --show-prompt` — print only the exact initial prompt that would be sent, labeled `explicit (--prompt)` or `generated (from --spec)`, and exit; same no-side-effects contract as `--no-exec`. <!-- trace:TASK-1467 | ai:claude -->
