...
> **The documenter derives entries from the spec graph, not from `--help` alone.** `python3 docs/cli/generate-entry.py <command>` assembles an *intent-bearing draft* of a command's entry by combining three sources: `--help` (the **what** / fact source), the command's `interface_changes` deltas (the **delta** a spec introduced — resolved from the `aida-store` branch by reusing the doc-intent gate's resolver), and the shaping spec's description (the **why** / intent). A command "shaped" by a spec is one whose `interface_changes` block names `aida <command>`. The draft strips SPEC-IDs from its prose (they survive only in a trailing `<!-- doc-intent: shaped by ... -->` provenance comment the leak check ignores), so it drops into a chapter without tripping the no-SPEC-ID rule. `--json` emits the raw `{command, help, shaping_specs}` payload for `aida-tutor`; `--list` shows every command some spec's `interface_changes` names. This is the consume half of the doc-intent protocol: the spec is the source of intent, and docs *derive* from the graph rather than re-determining the surface blind.
