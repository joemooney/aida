...
The **night shift**: a scheduler-run check that keeps drain waves going while no seat is awake. It runs as the `night-shift` substrate job of `aida schedule tick` (so it rides whatever driver `aida schedule install-cron` installed) and is **off unless enabled for this clone**.
...
Verbs: `aida shift tick [--dry-run] [--json]` (`--dry-run` prints the exact wave command, the specs, every guard with its value and where the switch was read, and writes nothing — the preflight), `aida shift status [--json]`, `aida shift enable`, `aida shift disable`, `aida shift resume`.

**The switch is per clone, and only a person can turn it on.** `aida shift enable` and `aida shift resume` refuse unless run at an interactive terminal outside agent output mode, and ask for an explicit yes; `aida shift disable` never asks. `aida shift enable` writes `enabled = true` to `~/.aida/shift-local.toml` under this repo's path — never to the committed `.aida/config.toml`, where an `enabled` key is ignored — and registers the `night-shift` job (harmless in any clone that has not enabled it). Tunables live under `[shift]` in `.aida/config.toml` or the local layer: `wave_size` (6), `max_failures` (2), `budget_stop_pct` (80), `daily_token_budget` (the watchdog's), `wave_token_budget`, `max_runtime` (`3h`), `max_waves_per_day` (8), `load_per_cpu` (1.5), `batch` (drain a pre-tagged batch instead of auto-tagging). `allow_uncovered_vendors` is read from the local layer only.
