...
- **substrate** — `command = "session reap"` (an allow-listed `aida` subcommand: `cache verify`, `session reap`, `queue gc`, `notify check`, `doctor`, `doctor check performance --fail-on-findings`, `doctor check remote-drift --fail-on-findings`, `doctor check stale-remote-branches --fail-on-findings`, `doctor check disk-headroom --fail-on-findings`, `fetch --code-only`, `store compact`). Needs no LLM. `aida schedule tick` runs the due ones (the per-turn hook calls `aida schedule tick --hook`, which skips network-touching jobs) or paste `aida schedule emit-cron` into a real crontab. Idempotent, quiet when idle. The `--fail-on-findings` shapes are **gates**, not reports: a non-zero exit emits `CronJobFailed`, which a paired `on = ["CronJobFailed"]` seat job turns into a due item — see `hub-drift-guard`, `stranded-branches-guard` and `disk-headroom-guard` in the scaffolded `[schedule]` section (commented by default; this repo runs all three enabled) for the worked pattern. <!-- trace:STORY-1367 | ai:claude -->
