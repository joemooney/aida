...
- **substrate** — `command = "session reap"` (an allow-listed `aida` subcommand: `cache verify`, `session reap`, `queue gc`, `notify check`, `doctor`, `doctor check performance --fail-on-findings`, `doctor check remote-drift --fail-on-findings`, `doctor check stale-remote-branches --fail-on-findings`, `doctor check disk-headroom --fail-on-findings`, `doctor check runaway-seats --fail-on-findings`, `fetch --code-only`, `store compact`). Needs no LLM. `aida schedule tick` runs the due ones (the per-turn hook calls `aida schedule tick --hook`, which skips network-touching jobs) or paste `aida schedule emit-cron` into a real crontab. Idempotent, quiet when idle. The `aida doctor check <category> --fail-on-findings` shapes are **gates**, not reports: a non-zero exit emits `CronJobFailed`, which a paired `on = ["CronJobFailed"]` seat job turns into a due item — see `hub-drift-guard`, `stranded-branches-guard` and `disk-headroom-guard` in the scaffolded `[schedule]` section (commented by default; this repo enables `hub-drift-guard` and `disk-headroom-guard`, and ships `stranded-branches-guard` disabled until its remote-branch backlog is healed) for the worked pattern. <!-- trace:STORY-1367 | ai:claude -->
  ...
  **Runaway-seat watchdog** (`watchdog` job, `aida doctor check runaway-seats`): a zero-token substrate check that reads the trailing day of this project's Claude Code transcripts (`~/.claude/projects/<slug>/`, for the project root and every worktree) and its headless logs (`.aida/headless-logs/`, `.aida/burndown/`), and trips per session on wake rate, token rate, the same injected prompt repeated (or firing faster than its declared cron), a mostly no-op reply ratio, a per-call context above `restart_context_tokens`, too many auto-compactions, or a model-side mailbox poll — plus the project's 24h token spend against `daily_token_budget`. Each finding names the session, its seat and pid when the agent registry knows them, the rule, the measured value, the threshold and the window. Reads are incremental (a per-file watermark under `.aida/watchdog/`) and capped by bytes and time; evidence it cannot read, or a run its budget cut short, is a finding (`evidence-unknown` / `evidence-partial`), never a clean pass. Thresholds live under `[watchdog]` in `.aida/config.toml`: `window_minutes` (60), `max_wakes_per_hour` (120), `max_tokens_per_hour` (100000000), `daily_token_budget` (2000000000), `budget_alert_pct` (50), `max_repeated_prompts_per_hour` (30), `max_idle_ratio_pct` (80), `idle_min_replies` (20), `noop_reply_max_chars` (60), `restart_context_tokens` (300000), `max_compactions_per_day` (3), `max_bytes_per_file`, `max_total_bytes`, `time_budget_ms` (10000). <!-- trace:STORY-1462 | ai:claude -->
