...
**Mail latency.** Each tick measures, per recipient, how long the oldest unread message has waited. Past `mail_latency` (`30m`) it notifies the operator through `aida notify` (never the mailbox or a chat), once per episode per recipient, again only after that recipient's mail is read and falls behind again. <!-- trace:TASK-1492 | ai:claude -->
