...
- `--to <STATUS>` / `--from <STATUS>` — only status transitions into (`--to`) or out of (`--from`) that status, newest first: `aida history --to approved --since 7d` is what got approved this week, `aida history --from in-progress --to approved` is work bounced back from in progress, and `--from` alone matches any transition leaving that status. Status values take the spellings `aida edit --status` does (`in-progress`, `needs-attention`, any case), plus `accepted` for approved; an unknown status is refused with the valid set. A transition must change the status, so `--to completed` lists exactly what `--shipped` does. <!-- trace:TASK-1512 | ai:claude -->
