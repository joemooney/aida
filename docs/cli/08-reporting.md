...
- `--max-commits` — bounds how far back it walks the orphan branch. The knob for "this is slow / I only care about recent." `--full`/`events` and a multi-spec `--json` default this to `(limit*5).max(50)` when you don't set it; the digest, the single-spec views, and the feed that `--shipped`, `--status-changes`, `--comments` or `--oneline` switch on default to 250. See Gotchas below for what happens when that default runs out.
