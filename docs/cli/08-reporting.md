...
- `--shipped` — the "did my ship register?" view: only transitions into Completed (merged to the default branch), newest first. Any prior status counts — a merge usually moves a spec straight from In Progress to Completed, older ships went Done → Completed — and a spec reopened and completed again shows each completion. <!-- trace:BUG-1636 | ai:claude --> Distinct from `--all` (a recency-blind dump of every terminal spec) — `--shipped` answers a question, `--all` widens the net.
