...
**Mental model.** `reconstitute` is the round-trip verifier of the reconstitution loop. A headless probe agent gets *store-only* context — the spec, its linked decisions, `[aida:sem]` micro-decisions, graph context and the *names* of the symbols its trace comments point at — and no source at all (it runs in an empty scratch directory outside the project and is told not to read anything). It regenerates the tests the criteria imply; a second pass judges, per criterion, whether each real traced test has a regenerated counterpart (matched / partial / missing, with a reason). The per-criterion divergence lines are the product. The probe also runs a **baseline arm (arm A)**: the same task given only what a project without AIDA keeps once the code is gone — git log subjects touching the relevant files (SPEC-IDs stripped), the README, and file and symbol names — and none of the spec, criteria, decisions, micro-decisions or graph. The result is the pair and the difference between them: arm A recall, arm B (store) recall, and the delta (B minus A, in percentage points), which is what AIDA added for this spec. Arm B is never shown alone. The report lists exactly what the baseline was given and withheld, and keeps its brief for audit. If either arm cannot be computed, it and the delta read "unknown", never zero. Recall is measured against what was captured, so **coverage** is reported next to it and never combined with it: the share of Rust tests carrying a trace, of open work specs with acceptance criteria, of criteria with a traced test, and of commits with a SPEC-ID trailer. The report names its validation subject; a run inside AIDA's own repository is labelled a lower bound, because that repository predates its own capture discipline. Recall scores are heuristic and are always labelled so. Real tests the store could not reproduce are written as harvest candidates; confirm them with `aida harvest <ID> --from <file>`. The probe never writes to the spec.
...
**Key options (rationale only).**
- JSON — the full report (both arms, delta, baseline inputs, coverage, validation subject, matches, untested criteria, unanchored tests, candidates file) for scripts and trend tracking. The older `score_*` fields remain and hold arm B's numbers.
- Dry run — print both probe briefs (store and baseline); useful to confirm neither carries source before spending an agent run.
- Yes — confirm the agent runs when stdin is not a terminal. The command prints how many headless runs it plans (up to four) and refuses to launch them unattended without this flag.

**Gotchas.** Up to four headless agent runs per probe (regenerate, then judge, for each arm). A failed run in either arm does not fail the report; that arm and the delta show as unknown. The README given to the baseline is first stripped of trace comments, SPEC-IDs, `[aida:…]` markers and `.aida` paths, so it carries nothing the baseline is meant to be denied. Judging is agent-based, so treat the score as a heuristic and read the reasons. Isolation is by construction of the brief plus an empty working directory, not a sandbox: a tool-using agent that ignores its instructions could still look around, so the guard is the brief (a test pins that it carries no test source). Probe artifacts live under `~/.aida/reconstitute/`; each run removes directories older than seven days and retains at most the newest 20 runs per machine. Harvest candidates live under the project's gitignored `.aida/harvest/` directory and are retained until manually reviewed because they may contain unconfirmed proposals.
