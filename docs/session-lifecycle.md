...
**The live-but-finished case — NOTIFY.** The reap is the right answer for a *headless* agent, which exits when its work lands. An *interactive* session doesn't: after its spec merges it sits idle at the prompt, holding its worktree as cwd, so the predicate's "process exited" gate is `false` and the reap correctly leaves it in place. That session is not stuck — it just doesn't know it's free to go. So the same pass that would reap it were it dead instead **notifies** it: a mailbox FYI (`intent: fyi` — surface-only, no action) addressed to the operator handle, reading *"spec X merged and its branch is finished; you are free to exit — the worktree reaps automatically once your process exits."* The session sees it on its next prompt turn through the per-turn `aida awaiting` unread-mail notice — the existing coordination surface, no new channel. The notice fires **once per session** (a per-session sentinel under `.aida/session-notices/` dedups it), never scrapes the terminal, and never closes the agent: it is detect-and-**notify**, the exact mirror of the reap's detect-and-leave. This is the interactive half of FR-284; the reap is the headless half.

Everything else the worktree GC refuses, the reap refuses too: dirty worktrees, locked worktrees, and branches carrying unique unmerged commits are never removed (the reap runs the very same classifier), and a session that fails any of those gates is never notified either — a dirty session is not "done." `--dry-run` previews (including which live sessions *would* be notified), `--json` reports (a `notifiable` array alongside `reapable`/`skipped`), and the pass also runs automatically after an AIDA-managed merge lands so the common autonomous path needs no manual step at all.
