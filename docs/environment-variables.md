...
| `AIDA_RED_RUN_BUDGET_SECS` | Wall-clock budget for the pre-implementation red run: when a drain's implementer phase starts, the spec's criterion-traced tests are run once and recorded under `.aida/red-runs/<SPEC>.json`. `0` disables it. Read at `aida-cli-lib/src/criteria_red_run.rs`. | `300`. | user | process env |
