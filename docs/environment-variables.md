...
| `AIDA_WORKER_CI_ABSOLUTE` | Absolute hard ceiling (seconds) for the CI-wait loop; overrides `[drain] ci_absolute`. Bounds the total wait even while an in-flight check keeps re-arming the idle timer. Evaluated before the idle timer. `0` disables the ceiling. trace:TASK-968 trace:BUG-1275 | `5400` (90 min). | user | process env or `.aida/config.toml` |
