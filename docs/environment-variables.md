...
| `AIDA_ALLOW_STALE_BINARY` | Lets a drain launch (`queue work --auto-complete`, `burndown run`) on a dev build of the AIDA workspace whose embedded git SHA is not the local default-branch HEAD (behind, diverged, or unknown). Without it the wave-launch freshness gate refuses and names `make build-fast` as the fix. Same effect as `--allow-stale-binary`; the bypass is recorded as `freshness_gate_bypassed` in `.aida/drain.lock`. Released binaries and downstream projects are never gated. Read at `aida-cli-lib/src/freshness_gate.rs`. trace:STORY-1414 | unset = gate enforced. Override with `1`/`true`/`yes`/`on`. | user | process env |
