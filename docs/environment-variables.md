...
| `AIDA_RED_RUN_BUDGET_SECS` | Opt-in wall-clock budget for the pre-implementation red run: when a drain's implementer phase starts, the spec's criterion-traced tests run once in the lane's own fresh worktree (per-lane `CARGO_TARGET_DIR` inside it, never the main checkout) and the result is recorded under `.aida/red-runs/<SPEC>.json`; a timeout is recorded as `timed-out`. `0` disables it; when unset, `[drain] red_run = true` in `.aida/config.toml` enables it with a 120s budget. Read at `aida-cli-lib/src/criteria_red_run.rs`. | unset = off. | user | process env |
