...
| `AIDA_HOME` | Redirects the global home (`~/.aida`) lookup — used for cross-platform isolation and deterministic test paths. Also relocates the machine-global state files under `~/.aida/`: `node.toml`, `agents.toml`, `presence.toml`, `solo.toml`, `usage.jsonl`, and the STORY-769 last-human-input oracle files `~/.aida/turn-clock/<session-id>.toml` (per-session last-prompt stamp; when no home dir resolves at all, these fall back to `$XDG_RUNTIME_DIR/aida-turn-clock/`). | unset = `dirs::home_dir()`. | dev / test | process env |
