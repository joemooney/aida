...
| `AIDA_FOCUS` | Overrides the active focus context (an epic/spec id) that scopes `aida list` / `aida status` / `aida queue list` to that spec's subtree (STORY-706), and is the scope the focus-scope drift guard checks work-start against (see `[focus] out_of_scope` below, STORY-717). Highest-precedence tier above the `.aida/focus` marker; a blank value falls through to the marker. | unset = use `.aida/focus`, else no focus. | user (or `aida focus <spec>` writes the marker) | process env |
...
| `AIDA_IDENTITY_GUARD` | Policy for the cross-identity guard (TASK-1150): when a queue add/remove or disposition-lease acquire/release would operate on a queue/lease owned by a *genuinely different* user id than this shell's identity on one machine (`user-a` vs `user-b` — not just a case variant, per TASK-951), it surfaces the mismatch instead of silently crossing identities. Distinct from `AIDA_TEAM_REQUIRE_USER`, which guards the shared `"default"` fallback in a team; this guards two *distinct* ids on one box. | `warn` (surface on stderr, proceed). Set `refuse` to block the op (non-zero exit), or `off` to silence. | user | process env |
...
| `AIDA_ZEN_NO_AI` | Forces the `aida zen "<thought>"` free-text front door to skip the AI draft and capture the thought verbatim (offline / deterministic). | unset = use the AI draft when Claude CLI is reachable. Enabled with `1`. | user | process env |
...
| `AIDA_HEADLESS_VENDOR` | Selects which vendor's headless CLI the orchestrator drain spawn launches — `claude` (`claude -p …`) or `codex` (`codex exec …`). Takes precedence over `[orchestrator] headless_vendor`; an unrecognized value is ignored and the config/default applies. Read at `aida-cli/src/session.rs` (`resolve_headless_vendor`). The per-invocation `--vendor`/`--agent` flag on `aida zen` / `aida queue work` / `aida burndown run` sets this (and an in-process override tier that wins over even a pre-set value) so a single drive can pick the implementer vendor without exporting the env (TASK-1116). trace:STORY-683 trace:TASK-1116 | unset = `[orchestrator] headless_vendor`, else the uniform `[agents] vendor` knob (project `agents.toml` > project `config.toml` (tracked/worktree-inherited) > global `agents.toml`; STORY-761/BUG-704), else `claude`. | user | process env |
| `AIDA_AGENT_CMD` | Overrides the PROGRAM invoked at every HEADLESS agent spawn site (drain phase, headless resume, advisor tier, and the `claude agents --json` liveness query) — the vendor binary (`claude` / `codex`) is swapped for this executable while the full argv (all flags + the brief/prompt positional) is passed through unchanged, so a scenario mock receives exactly what the real vendor CLI would (EPIC-59). Only the executable swaps; the flags are still chosen per vendor. Interactive TTY launches are unaffected. Read at `aida-cli/src/session.rs` (`resolve_agent_program`). trace:TASK-1081 | unset = the native vendor binary. Empty/whitespace falls back to the native binary. | user / dev / test harness | process env |
| `AIDA_TUI_VENDOR` | Selects which vendor CLI a hosted `aida tui` tab launches behind `aida queue work` — `claude` (default) or `codex`. Takes precedence over `[tui] vendor`; an unrecognized value is ignored and the config/default applies. Codex has no caller-minted session id, so a Codex tab hosts a fresh session with no `--session-id`/`--resume`. Read at `aida-tui/src/config.rs` (`TuiConfig::load`). trace:TASK-895 | unset = `[tui] vendor`, else the uniform `[agents] vendor` knob (project `agents.toml` > project `config.toml` (tracked/worktree-inherited) > global `agents.toml`; STORY-761/BUG-704), else `claude`. | user | process env |
| `AIDA_TUI_LIVENESS_TTL_SECS` | Freshness window (whole seconds) for the `aida tui` cockpit's per-row liveness glyph before it re-runs the `aida ps` probe. Higher = fewer probes (the probe is ~1.3s; BUG-676). Absent / non-numeric / `0` falls back to the 20s default. The probe also only fires when a liveness-showing scope is on screen and never while a previous probe is in flight. Read at `aida-tui/src/redesign/liveness.rs` (`probe_ttl`). trace:BUG-676 | unset = `20`. | user | process env |
| `AIDA_TUI_REDESIGN` | Selects the `aida tui` rendering model: the EPIC-54 action→target command-palette redesign (default) vs the legacy PTY-supervisor TUI. **Now default-ON (TASK-1051)** — an explicit opt-OUT (`0`/`false`/`no`/`off`, case-insensitive) selects the legacy TUI; unset/empty/any other value renders the redesign. Read at `aida-tui/src/redesign/mod.rs` (`enabled`, re-exported as `aida_tui::redesign_enabled`) — the CLI launcher gate (`aida-cli/src/main.rs`) and `aida_tui::run` share the predicate. trace:TASK-1051 | unset = redesign (default-on). | user | process env |
...
| `AIDA_BURNDOWN_BG_WAIT_CEILING_MS` | Overrides the bounded background-wait ceiling AIDA sets on every headless child it spawns (see `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS` below). Precedence: this variable → `[burndown] bg_wait_ceiling_ms` in `.aida/config.toml` (project, then `~/.aida/config.toml`) → the default. Any value below the 60 000 ms floor — including `0` — is clamped UP: wait-forever is deliberately unreachable, because it removes the safety valve that lets a genuinely wedged task surface. An unparseable value is ignored rather than fatal. trace:TASK-1169 | `2700000` (45 min — worst-case cross-platform CI plus margin). | user | process env |
| `AIDA_BURNDOWN_ORDER` | How a drain orders its unblocked ready set: `priority` (high before medium before low, ties broken by queue-insertion time) or `queue` (strict queue-insertion order — oldest `added_at` first, no priority reordering). `input` is a back-compat alias for `queue`. Top tier of the ladder: this variable → `[burndown] order` in `.aida/config.toml` (project, then `~/.aida/config.toml`) → `priority`. `aida burndown plan --order` / `aida burndown run --order` set it, and `burndown run` exports it so the headless drain's own re-resolves use the same order. An unrecognized value falls through to the next tier rather than failing the drain. trace:TASK-1175 | unset = config, else `priority`. | user / `--order` flag | process env |
...
| `AIDA_PANES` | Hosts each fanned-out drain implementer in its own titled terminal window instead of a background process. `tmux` runs each implementer in a window named by its spec under a dedicated `aida-drain` tmux session. Degrades gracefully to the background spawn (with a one-line notice) when not inside a tmux server or the backend is unavailable. | unset = background spawn. Set to `tmux`. | launch-path (`--panes` dispatch on `queue work` / `burndown run`) | process env |
...
| `AIDA_PR_SHIP_ALLOW_IN_DRIVE` | Lets `aida pr ship` merge while a drive is live, bypassing the drive-seat and drive-owned-PR guards (a deliberate in-drive direct publish). It does **not** bypass a merge-hold: a held PR still needs a human at a terminal, and a rework/recusal/untyped hold still has to be cleared with `aida merge-hold clear <PR>` first. | unset = guards active. Opt-in (`1`). | user | process env |
...
| `AIDA_LOCKING` | Overrides the `[locking] posture` config for the automatic advisor-lock enforcement gate (STORY-711 slice 2) — the commit-boundary bouncer that refuses/warns when the current worktree carries a different advisor's lock than the committing session's authorization token (read from the launch-context snapshot, not this env var). Wins over `.aida/config.toml`. | unset = config `[locking] posture`, else `context-aware` (TASK-958 default). Valid values: `context-aware`/`context`, `off`, `warn`, `enforce`. `context-aware` hard-blocks a `Refused` commit only under a **corroborated live drain** (`AIDA_AUTO_COMPLETE` set AND its `AIDA_AUTO_COMPLETE_TOKEN` names a live orchestrator run), warns when solo or under a bare/stale `AIDA_AUTO_COMPLETE`; `off` disables gating; `warn`/`enforce` always warn/block on a mismatch regardless of drain. | user | process env |
| `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS` | Read by Claude Code (not AIDA): how long a headless `claude -p` session waits for its still-running background tasks at turn end before reaping them. **AIDA SETS this on every headless child it spawns** — the burndown launcher (quiet + `--verbose`), the orchestrator's headless phases, the advisor tier, the fix-forward pass, and the resume path — so a hand-typed `aida burndown run` behaves identically to a daemonized one (the divergence behind the 2026-07-18 limp incident). Value comes from `AIDA_BURNDOWN_BG_WAIT_CEILING_MS` / `[burndown] bg_wait_ceiling_ms` / the default, clamped to a non-zero floor. The load-bearing CI/merge wait no longer depends on this at any value — it runs in the launcher process (ADR-21) — so this is defense-in-depth. trace:TASK-1169 | AIDA sets `2700000` (45 min). | AIDA (launch path) | process env |
...
| `AIDA_WORKER_CI_IDLE` | Idle window (seconds) for the orchestrator/worker CI-wait loop (`wait_for_ci_terminal`); overrides `[drain] ci_idle`. The deadline re-arms while a required check is queued/running, or when a check appears/transitions, the check set changes, or the PR head / base tip advances. Only absence of all such progress expires the window. `0` disables the idle timer. The 20-minute default exceeds the measured 705-second p95 of the latest 97 completed GitHub `CI` runs on 2026-09-19 (`gh run list --workflow CI --limit 100`). trace:TASK-968 trace:BUG-1275 | `1200` (20 min). | user | process env or `.aida/config.toml` |
| `AIDA_WORKER_CI_ABSOLUTE` | Absolute hard ceiling (seconds) for the CI-wait loop; overrides `[drain] ci_absolute`. Bounds the total wait even while an in-flight check keeps re-arming the idle timer. Evaluated before the idle timer. `0` is **clamped to 14400 (4h)** with a warning rather than disabling the ceiling — an unbounded wait is a real hang risk (TASK-1453). At that ceiling, if a concluded failure is already known in the last rollup, the wait reports Red naming it (the still-stuck check listed as pending) instead of NoSignal; a rollup with nothing but pending checks still reports NoSignal. trace:TASK-968 trace:BUG-1275 trace:TASK-1453 | `5400` (90 min); `0` clamps to `14400` (4h). | user | process env or `.aida/config.toml` |
...
| `AIDA_DRAIN_LOCK_STALE_SECS` | Age (seconds) past which a still-claimed drain lock is treated as stale and reclaimed even if its PID + process-start identity still matches (wedged-process backstop). Also the TTL of the shared cross-clone drain claim (STORY-638). | `1800`. | user | process env |
...
| `AIDA_DRAIN_RESUME_PHASE` | Carries the phase recorded by the latest `SpecShelved` event from `aida drain resume <SPEC>` into its child `queue work --from-pr` process. The child reconciles it with current PR reality and never rewinds to an earlier phase. | unset = normal reality-based `--from-pr` selection. | launch-path (`aida drain resume`) | process env |
...
| Variable | What it does | Default | Who sets it | Scope |
| --- | --- | --- | --- | --- |
| `AIDA_AUTO_COMPLETE` | Bare "this session is orchestrator-spawned" flag (only trusted with a matching token). Also the drain signal the `context-aware` `[locking]` posture (TASK-958) reads at commit time — but only a **corroborated** live drain (this flag AND `AIDA_AUTO_COMPLETE_TOKEN` naming a live orchestrator run) makes a `Refused` commit hard-block; a bare/stale flag only warns. | unset = standalone / solo. Set `1`. | launch-path | process env |
...
| `AIDA_RED_RUN_BUDGET_SECS` | Opt-in wall-clock budget for the pre-implementation red run: right after `aida queue work` creates an implementer lane's worktree and before the agent launches, the spec's criterion-traced tests run once in that fresh lane (HEAD at the default-branch tip) (per-lane `CARGO_TARGET_DIR` inside it, never the main checkout) and the result is recorded under `.aida/red-runs/<SPEC>.json`; a timeout is recorded as `timed-out`. `0` disables it; when unset, `[drain] red_run = true` in `.aida/config.toml` enables it with a 120s budget. Read at `aida-cli-lib/src/criteria_red_run.rs`. | unset = off. | user | process env |
...
| `AIDA_QUIET` | Suppresses non-essential informational stderr notices (e.g. the `AIDA_STORE` set-but-unusable fall-through notice, and the BUG-568 shared-store-multi-repo "scan covered only the local repo" warning emitted by auto-bump / reconcile-status / linkage / trace-comment scans). Any value other than unset / empty / `0` / `false` enables it. **Presentation-only**: `aida agent new` does NOT propagate this from the launching shell into a spawned agent — see the `AIDA_OUTPUT_FORMAT` row. | unset = notices shown. | user / scripts | process env |
| `AIDA_HOME` | Redirects the global home (`~/.aida`) lookup — used for cross-platform isolation and deterministic test paths. Also relocates the machine-global state files under `~/.aida/`: `node.toml`, `agents.toml`, `presence.toml`, `solo.toml`, `usage.jsonl`, the telemetry logs `auto-complete.jsonl`, `field-study.jsonl` and `rule-violations.jsonl`, the per-clone night-shift switch `shift-local.toml`, and the STORY-769 last-human-input oracle files `~/.aida/turn-clock/<session-id>.toml` (per-session last-prompt stamp; when no home dir resolves at all, these fall back to `$XDG_RUNTIME_DIR/aida-turn-clock/`). | unset = `dirs::home_dir()`. | dev / test | process env |
| `AIDA_STORE_GC_AUTO` | Overrides the `gc.auto` threshold AIDA sets on the orphan-store repo so git auto-repacks the shared object store far sooner than its 6700-loose-object default — a heavy drive bloats loose objects below 6700, which never triggers git's gc yet slows every targeted store commit ~4x (TASK-1033). Set on worktree attach + at sync chokepoints (`aida pull`, `aida db sync`), which also run a best-effort `git gc --auto`. `off`/`disabled`/`none`/`0` disables AIDA's self-maintenance; a positive integer sets that threshold; garbage falls back to the default. Best-effort + idempotent — a config-set failure never breaks the store. | `500`. | user | process env |
| `AIDA_STORE_GC_AGGRESSIVE_PACKS` | Pack-count high-water mark at which AIDA's opportunistic store maintenance (the same sync chokepoints as `AIDA_STORE_GC_AUTO`) escalates from a cheap `git gc --auto` to a deep `git gc --aggressive`, consolidating the many packs a long never-compacted orphan history accumulates (STORY-733). Aggressive is expensive but self-throttling — it collapses every pack into one, so it won't re-fire until packs climb back past the threshold. Non-destructive (no history rewrite). `off`/`disabled`/`none`/`0` disables auto-aggressive; a positive integer sets the threshold; garbage falls back to the default. Run it on demand any time with `aida store compact` (alias `aida store gc`). | `50`. | user | process env |
...
| `AIDA_CACHE_LOCK_EXPECTED_SECS` | Expected duration, in seconds, of one cache write, recorded in the cache lock-info as a soft deadline. Diagnostic only: an overrun is reported (lock error note, `aida doctor` finding) but is never used to reclaim a live lock. | `120`. | user | process env |
| `AIDA_STORE_LOCK_TIMEOUT_SECS` | Seconds a write waits for the requirements store's write lock (`.aida/store-write.lock` in the store) before giving up with an error. Every store write holds this lock, so another AIDA command writing the store at the same moment makes this one wait. | `120`. | user | process env |
...
| `AIDA_HISTORY_CACHE` | Switches the rebuildable history index behind `aida history` and the MCP `history` tool on or off. Off means every query reads the store's git log directly; answers are the same, only slower. When off, the human fallback footer is not shown and MCP reports `source: git-walk`. | enabled. `0`/`false`/`off`/`no` disables it. | user / test | process env |
| `AIDA_HISTORY_INDEX_BUDGET_MS` | Milliseconds of inline indexing one history query may spend catching the history index up before it answers (or falls back to reading git directly). `aida cache rebuild --history` ignores it. | `1500`. | user | process env |
...
| `AIDA_EVENTS_DISABLE` | Kill switch for the drain event stream: when truthy (anything but empty/`0`/`false`/`no`/`off`), `events::emit` is a no-op and nothing is appended to any project's `.aida/events.jsonl`. Exists for TEST HARNESSES (BUG-770) — an integration test spawns the real binary in a child process, where an in-process seam can't reach and `cfg!(test)` is false, so exporting this once around the test run guarantees no test at any nesting depth can append synthetic events to a developer's real supervision stream. Do not set it for a real drain: a watcher (`aida watch`, `event_wait`) would then never wake. | unset (stream enabled). | test | process env |
...
| `AIDA_PUSH_QUIET` | Suppress the non-blocking "N uncommitted change(s) not included in this push" notice (same effect as `aida push --no-notice`). **Presentation-only**: `aida agent new` does NOT propagate this from the launching shell into a spawned agent — see the `AIDA_OUTPUT_FORMAT` row. | unset = notice shown when the code tree is dirty. Truthy values suppress; `0`/`false`/`no`/empty do not. | user | process env |
...
| `AIDA_SCHEDULE_INVOKER` | Set on the `schedule tick` invocation itself (not on jobs it spawns — see `AIDA_SCHEDULE_CHILD` above) to tag which driver is calling it: `cron` on the line `aida schedule install-cron` generates, `systemd` in the unit `aida schedule install-systemd` writes. Recorded as `schedule_source` in local usage telemetry (`~/.aida/usage.jsonl`) so a config error is traceable to its driver. The per-turn hook doesn't need it — its own `--hook` flag is already self-identifying; unset with no `--hook` reads back as `manual`. Read-only marker; never set it by hand. <!-- trace:BUG-1600 | ai:claude --> | unset. | launch-path (`aida schedule install-cron` / `install-systemd` / `aida init`) | process env |
...
| Variable | What it does | Default | Who sets it | Scope |
| --- | --- | --- | --- | --- |
| `AIDA_EDITOR` | Which editor `aida edit <spec>` (with no field flags, at a TTY) opens on the git-commit-style markdown buffer of the spec's title + description. First tier of the resolution cascade: `AIDA_EDITOR` → `VISUAL` → `EDITOR` → platform default (`vi` on unix, `notepad` on Windows). Multi-word values (`code --wait`) work — the editor is launched through the platform shell, like git. | unset = fall through to `VISUAL` / `EDITOR` / platform default. | user | process env |
| `AIDA_GLYPHS` | Selects the glyph rendering profile. Values: `unicode` (emoji/unicode, the default) or `ascii` (curated ASCII fallbacks). Highest-priority tier of the selector: env > project `.aida/config.toml` `[ui] glyphs` > user `~/.aida/config.toml` `[ui] glyphs` > default `unicode`. **Presentation-only**: `aida agent new` does NOT propagate this from the launching shell into a spawned agent — see the `AIDA_OUTPUT_FORMAT` row. | `unicode`. | user | process env |
| `AIDA_AGENT_OUTPUT` | Forces AGENT-MODE output for the agent-ergonomics surfaces (TASK-970): bare `aida` routes to the `aida status` snapshot + top queued (not the getting-started menu), and bare `aida list` applies a default 30-row cap with a `count: N of M` header + widen hint. **TASK-964**: in agent mode `aida list` / `show` / `queue list` / `status` also render as token-efficient **TOON** (a compact `name[N]{fields}:` tabular encoding + `key: value` scalars) with a minimal default schema instead of the emoji/table or verbose JSON — `aida list --fields a,b,c` widens the columns; `aida show --full`/`-v` un-truncates the description. Agent mode is normally auto-detected (non-TTY stdout = agent); this env var forces it. Truthy (`1`/`true`/`yes`/`on`) forces agent mode even at a TTY; falsey (`0`/`false`/`no`/`off`/empty) forces the human path even when piped. Does NOT change `--short`/`--json`/`--tree` shapes, and an explicit `--limit`/`--all` always overrides. **Presentation-only** (TASK-1482): `aida agent new` does NOT propagate this from the launching shell into a spawned agent — the child decides its own agent-mode-vs-human posture from its own stdout, same as any other `aida` invocation. Explicit opt-in: `[agents] inherit_env = ["AIDA_AGENT_OUTPUT"]` in `agents.toml` (project overrides user-global, same precedence as `[agents] bypass`/`contained`). `new --no-exec` / `--show-context` both print a `presentation_env` line naming what was stripped vs. kept. | unset = auto (non-TTY stdout → agent mode). | user / agent launcher | process env |
| `AIDA_OUTPUT_FORMAT` | **Pins the output format** so a captured/piped `aida` invocation gets the format it expects instead of silently switching. Because agent mode is auto-detected from whether stdout is a TTY, `$(aida list ...)` command substitution and `aida list \| grep …` pipes otherwise emit TOON where an interactive run showed the human table — which broke real user scripts (BUG-707). Values: `human` (alias `table`) = the human table/emoji output; `toon` (alias `agent`) = the compact agent TOON output; `json` = the machine-JSON projection where the command supports it (`list`/`search`/`status`). Honored uniformly by every render path. The global `--format <human\|toon\|json>` flag is the per-invocation equivalent and **wins over this env var**; both win over `AIDA_AGENT_OUTPUT` and the TTY default. When output auto-switches to TOON with neither a pin nor `AIDA_AGENT_OUTPUT` set, a one-time stderr hint points here (STDERR only, so piped STDOUT stays clean). **Presentation-only** (TASK-1482): `aida agent new` strips this (and `AIDA_AGENT_OUTPUT`/`AIDA_GLYPHS`/`AIDA_QUIET`/`AIDA_PUSH_QUIET`) from the spawned agent's environment by default — a captured/piped *launching* shell must not force the *child's* renderer. Same `[agents] inherit_env` opt-in as `AIDA_AGENT_OUTPUT`. | unset = auto (human at a TTY, TOON when piped/captured). | user / script author | process env |
...
| `AIDA_PERF_GATE_ALL_BINARIES` | Turns OFF the `doctor check performance` binary-lineage scope (BUG-1572), restoring the pre-fix behaviour where every `aida` binary that appended to the machine-global `~/.aida/usage.jsonl` counts toward the gate's denominator. Scoped ON by default inside a git repo: only calls served by a binary whose `binary_sha` is an ancestor of the current HEAD are judged, so one worktree lane on a stale or deliberately-regressed build cannot hold main's gate red. A sha git cannot place (absent, short, purged, or from another project) is COUNTED, never dropped. | scope ON. Opt out with `1`/`true`/`yes`/`on`. Also inert when HEAD cannot be resolved (not a git repo, git unavailable), where the gate runs unscoped. | user / CI | process env |
...
| `AIDA_WT_PS1_PREFIX` | The ambient `(wt:<FOCUS>) ` PS1 segment `aida worktree enter` splices in so a shell inside a scoped worktree is always visibly marked. Stripped + unset by `aida worktree exit`, which keys on this (and `AIDA_SESSION_ID`) rather than on cwd — a shell that carries the marker can clear it from anywhere. | set by worktree enter. | launch-path (shell code emitted by `worktree enter`/`exit`) | process env |
| `AIDA_WT_LEASE_FILE` | Path to the session lease file backing the `(wt:)` marker. The shell wrapper's prompt hook tests it once per prompt: when the file is gone (the session was ended from another shell) the marker is dangling, so the hook strips it and clears the carried session env instead of rendering a live-looking marker. | set by worktree enter when a session lease backs the worktree; explicitly unset otherwise. | launch-path (shell code emitted by `worktree enter`/`exit`) | process env |
| `AIDA_SHELL_WRAPPER` | Signals the `aida dev` shell wrapper is active — gates bare vs `eval`-wrapped auto-eval hints. Comma-separated list of the auto-evaled verb groups plus protocol/prompt-hook capabilities (`role,session,dev,worktree,worktree-exit,worktree-stale,eval-block`); the binary probes it per-capability to warn when an older installed wrapper can't eval a newer verb (or lacks the stale-marker self-heal). **`eval-block` is the protocol negotiation for the dedicated eval channel**: when advertised, the binary wraps shell payloads in `#aida:eval:begin` / `#aida:eval:end` markers and the wrapper evals only that block, so human stdout is never an eval candidate; when absent (older wrapper, or no wrapper at all) the binary emits the legacy bare payload unchanged. | unset = no wrapper. | launch-path (shell wrapper) | process env |
