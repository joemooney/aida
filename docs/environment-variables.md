...
| `AIDA_HEADLESS_VENDOR` | Selects which vendor's headless CLI the orchestrator drain spawn launches — `claude` (`claude -p …`) or `codex` (`codex exec …`). Takes precedence over `[orchestrator] headless_vendor`; an unrecognized value is ignored and the config/default applies. Read at `aida-cli/src/session.rs` (`resolve_headless_vendor`). trace:STORY-683 | unset = `[orchestrator] headless_vendor`, else the uniform `[agents] vendor` knob (project `agents.toml` > project `config.toml` (tracked/worktree-inherited) > global `agents.toml`; STORY-761/BUG-704), else `claude`. | user | process env |
