...
**A caution against overselling the sibling.** Traction must be read carefully — stars reflect hype as much as engineering ("era of vibe"). Beads has drawn pointed peer criticism: CLI bloat, slowness, and a *dual-store* design (a database kept in sync with a JSONL file by a background daemon) with reported data-corruption and version-migration breakage. Treat its on-disk model as *contested* — reports conflict between Dolt and SQLite+JSONL, pending direct verification. This is not a dismissal; the *idea* is widely credited. But it reframes AIDA's single-source git-YAML store (with a *rebuildable* cache and no daemon) as a deliberate architectural alternative to exactly that dual-store-with-daemon design — not merely a less-mature version of it.
