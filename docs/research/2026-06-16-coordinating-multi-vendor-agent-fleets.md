...
What this buys is a *better* claim, not a retraction. The pilot tested the easiest possible case — the rule is short, explicit, and the commit *is* the task, so the invariant sits right at the moment of action. The original P1 evidence came from *semantic, multi-step, cross-cutting* invariants (don't approve your own spec, trace every change, don't merge over a RequestChanges) that bite far from where they are stated. Set the pilot beside the competitive bake-off (§7), where Codex *skipped* a named rule — the `--ai` gate buried as one line of fine print in a 19-line brief — and lost: same class of model, opposite rule-adherence. The difference is not the model and not "rule vs gate"; it is **how far the invariant sits from the moment of action.**

> **Restated P1 — grade (M)(D), now with experimental support.** *Substrate-as-bouncer is a **selective** discipline, not a blanket one. The real variable is **attention-distance**: a capable model reliably honors invariants that are immediate, explicit, and at the point of action; it drops invariants that are buried, deferred, or competing for attention across a long autonomous run — and that is where a programmatic gate earns its place.* Gate the high-attention-distance invariants (the ones that bite far from where they are stated, or run unattended — merge-over-RequestChanges, approve-your-own-spec, the drain safety rails); trust the model on the immediate point-of-action ones. Gating everything is over-engineering; gating nothing is the Codex `--ai` miss. The skill is choosing *which* invariants the substrate must enforce. The decisive follow-up (the §13 I2 ablation: a harder, semantic, non-format invariant) predicts Arm-R compliance *drops* as attention-distance rises — i.e. P1 *holds* exactly where the restated form says it should.
...
| …and the converse: *a stated rule is insufficient against a capable agent* (the blanket form) | **Falsified for immediate, in-context rules** — the gate-vs-rule pilot found 100% (10/10) rule-only compliance on the commit-format invariant; the gate did no work. The real variable is attention-distance, not enforcement mechanism | P1 (scoped: gate selectively) |
...
**Benefits (observed).**
- *Control of the bouncer.* You can place gates and invariants (P1) exactly where a vendor will not, because the incentives differ — and the gate-vs-rule pilot sharpens this: the value is *control of where to gate*, gating the high-attention-distance invariants a vendor's in-context rules will leak, not gating everything.
...
2. **Gate vs rule ablation (P1).** The same invariant enforced as a CLAUDE.md rule vs a programmatic gate; measure violation rate per 100 agent-actions. *Piloted (`2026-06-18-gate-vs-rule-pilot.md`): on an immediate format invariant the gap was zero (100% rule-only compliance), scoping P1 to attention-distance.* The decisive open follow-up is **I2** — a harder, semantic, far-from-the-action invariant, where the restated P1 predicts the gap *reappears*.
...
| P1 | L2 | *Scoped:* gate selectively by attention-distance — far-from-action invariants need gates, point-of-action ones do not | gated transitions, merge-gate, trailer-completes; gate-vs-rule pilot | (M)(D) |
