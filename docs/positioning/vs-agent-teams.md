...
| **Quality gates** | **Hooks** that fire on team events | Commit-time trace enforcement, CI phase, reviewer phase, auto-bump-on-merge |
...
- **No cross-vendor reach.** Agent Teams coordinates *Claude* agents. AIDA's queue routes the same spec to a Codex or Antigravity session against one shared git-canonical store. A Claude-only coordination primitive structurally cannot be the substrate for a multi-vendor project.
