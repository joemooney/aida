...
- **No cross-vendor reach.** Agent Teams coordinates *Claude* agents; a Claude-only primitive doesn't reach a Codex or Antigravity session. AIDA's queue routes the same spec across vendors against one shared git-canonical store. *(Refresh 2026-07-19: cross-vendor routing is trust-tiered, not symmetric — Codex takes headless implementer work; Antigravity is dispatched draft-for-review via briefs only. SPIKE-76.)* Anthropic *could* extend Agent Teams cross-vendor — but that cuts against the incentive to keep coordination inside the Claude ecosystem, which is why the gap is likely to persist, not because it's technically unbridgeable.
