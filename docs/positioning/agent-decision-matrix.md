...
| Status / observability | Command-backed status line (`statusLine.command` can run e.g. `aida statusline`) — live, scriptable footer | Built-in TUI footer with a fixed (rich) field set; **no command-backed status line yet** (open upstream FR); `codex exec --json` gives a structured event stream for automation | `aida status` polling, a TUI, or tmux/statusbar integration — agent-independent | This is **UX, not correctness.** Losing the command-backed footer in Codex is a real but low-severity gap; rebuild the readout in tmux, a TUI, `aida statusbar --plain`, or by polling. Do not let an observability nicety drive a tooling decision. (parity inventory, section 2; porting doc, "the one genuine gap") |
