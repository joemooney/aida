...
| MCP | First-class MCP client (`.mcp.json`) | First-class MCP across stdio and HTTP, OAuth/bearer; `codex mcp add` | MCP server is the substrate's primary surface (the spec graph + coordination tools) | **MCP is the vendor-neutral layer.** This is the seam to standardize on regardless of agent. Register the same MCP server with each agent; the tool surface is identical. (porting doc + parity inventory both lean on this) |
