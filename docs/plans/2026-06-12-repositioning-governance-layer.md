...
- **D9 — RESOLVED (operator, 2026-06-12)**: README leads with the operator's own statement (canonical source language, recorded on ADR-4): *"make it easier to develop when you have many agents from different vendors and want to coordinate work and feature development as the system grows in complexity — capturing the intent, the why, that can get lost during implementation."* Lead emphasis = **multi-vendor coordination + intent/why preservation** (exactly the two verified surviving edges); **governance is the mechanism and the enterprise depth, not the lead word**. Trojan-horse survives: TUI stays the face; this is the named depth. D2's Beads-contrast line is demoted to the positioning papers (vs-beads.md), not the README lead.
