...
|---|---|---|
| **[2026-06-09-beads-gastown-substrate-competitor.md](2026-06-09-beads-gastown-substrate-competitor.md)** | Point-in-Time Snapshot | **First substrate-axis competitor**: deep-dive on `gastownhall/beads` (~24.4k ⭐, Dolt-backed typed task graph + hash IDs, drained by Gas Town). Verified AIDA-vs-Beads matrix with per-claim provenance. **Corrects the 2026-05-31 keystone**: typed-multi-vendor-graph is no longer AIDA-unique (Beads got there at scale) — re-centers the moat on the *surviving* edges (code↔spec trace enforcement, gated lifecycle + auto-bump, plain-git zero-infra, requirements-vs-issues framing). Proposes the corrected positioning line + a Beads import bridge as a Trojan horse. |
