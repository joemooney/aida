...
| **[marketplace-roster.md](marketplace-roster.md)** | **Living Roster** | **The field, by category**: the substrate (spec/issue/intent/memory-graph) and agent-orchestration projects AIDA tracks — so we build on / interoperate with prior art rather than reinvent it, and so any reviewer can see the landscape we measure against. Includes the discovery method that keeps the list from going vocabulary-blind. |
