...
**Living document** · Last updated: 2026-07-07 · Refresh cadence: with each ecosystem scan (see `README.md` · `signals-to-watch.md`)
...
>
> **Full landscape scans (June 2026):** the baseline ~50-player map is `docs/research/2026-06-17-agent-coordination-market-landscape.md`; the current material refresh is `docs/research/2026-06-26-agent-coordination-market-landscape.md`. This roster is the curated *lane view* (substrate / orchestration / standards + the platforms we ride); the dated scans are the point-in-time evidence.
...
| Project | What it is | Relation to AIDA |
|---|---|---|
| **Gas Town / Gas City** (`gastownhall`, ~15.9k★, Yegge) `[V]` | Fleet manager ("Kubernetes for agents"): Mayor/Deacon/Witness, Refinery merge-queue, Convoys, on Beads. **Gas City** = Gas Town rewritten as an SDK (composable "packs", v1.0 Apr 2026) | **Nearest orchestration competitor** + the richest durable coordination model. The 2026-06-26 refresh reports OSS cross-vendor support (Claude Code, Codex, Gemini, Copilot) plus Wasteland federation; quality/depth of non-Claude support remains hands-on-unverified. AIDA's edge is not scale or "cross-vendor exists"; it is the narrower bundle of front approval-gate + advisor-escalation + code↔spec traces + plain-git store. |
...
| **cmux** (`manaflow-ai`, ~23.9k★, GPL-3.0+) `[V]` | Native **macOS** terminal (Swift/AppKit on libghostty) purpose-built for parallel terminal agents: vertical tabs, per-pane notification rings, sidebar w/ branch+PR+ports, **socket API/CLI programmability**, embedded scriptable browser (agents verify their own web changes), renders Claude Code subagents/teams as native panes; agent-agnostic (Claude Code, Codex, OpenCode, Gemini, Aider, …); iOS beta; paid "Founder's Edition" (cloud VMs, voice). Self-positioned "a primitive, not a solution." | Hosting/multiplexing layer, **no substrate** (no work-item graph/IDs/traces/queue — verified 2026-07-07). Beads-scale distribution at the layer *under* AIDA; overlaps AIDA's TUI, not the moat. **Tripwire:** structured task/work-unit state atop its socket API = commoditizing orchestration from the distribution-rich end. **Note:** the "CMux" in `tui-prior-art.md` (zero-dep shell script, `cmux new <branch>`) does **not** match this project — name collision or a much earlier incarnation; disambiguate next scan. |
...
The **white space has narrowed** since the 2026-06-17 baseline: *cross-vendor + free + self-hosted + durable coordination* is no longer cleanly unoccupied because the 2026-06-26 refresh reports Gas Town OSS cross-vendor support and Wasteland federation. The defensible open slice is now more precise: **a plain-git, program-owned coordination record that combines typed requirements, machine-checkable code↔spec traces, lifecycle/history, pre-work authority, and cross-vendor access without making a vendor runtime or hosted control plane the source of truth.** The honest open risk is distribution and execution, not feature uniqueness.
