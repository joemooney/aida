...
- `add_requirement({title, ...})` → file a new spec
...
> **Schemas:** `outputSchema` descriptors are being added across all 21 tools in **TASK-440** (in flight). Until that lands, tools advertise `inputSchema` but the output shape is implicit (text envelope). If you do schema-driven tool consumption, expect Path A (descriptor-only) in the next ship; structured emission of `structuredContent` is the Path B follow-up (**STORY-399**).
...
That's the server. Configure your MCP client to connect to it. For a Codex client (subject to confirmation by **STORY-398**, the empirical roundtrip), the `.codex/` directory scaffolded by `aida init` contains the integration hints. **STORY-398 is the in-flight story that produces the authoritative `docs/agents/codex-mcp-setup.md`** with the working steps — read that doc when it lands (PR likely within the week of this brief's date).
...
|---|---|---|
| **TASK-440** | Adds `outputSchema` descriptors to the 21 MCP tools | High priority, in flight |
| **TASK-438** | Fixes TOCTOU race on `claim_task` (two concurrent claims could both succeed) | Approved, queued; not yet shipped |
| **BUG-310** | MCP-created specs not consistently visible to local CLI (real correctness gap) | High priority, queued |
| **STORY-398** | Empirical Codex roundtrip + `docs/agents/codex-mcp-setup.md` | High priority, in flight |
...
| **BUG-307** | Orchestrator auto-cleans dormant leases (reduces "lease stuck" friction) | High priority, queued |
| **BUG-311** | `aida queue work --steal` reliability fix for dormant-lease cleanup | High priority, queued |
...
*This brief was written 2026-05-22 by the AIDA project's live advisor for sibling-agent onboarding. As STORY-398 produces the authoritative `docs/agents/codex-mcp-setup.md`, that doc supersedes the "How to connect" section here.*
