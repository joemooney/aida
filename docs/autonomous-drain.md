...
The built-in resilient runner is `scripts/drain-loop.sh`: its main loop checks
whether the active role has queued work, then runs
`aida queue work "next${CHUNK}" --auto-complete …`. Inside AIDA, that enters
the `nextN` path (`handle_auto_complete_next_n`), whose
`RealNextNDriver::next_head` re-resolves the queue head on each iteration and
whose `RealNextNDriver::run_spec` calls `run_auto_complete` for the selected
